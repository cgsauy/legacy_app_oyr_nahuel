...
Private Sub chEsReclamo_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then
        If tReclamo.Enabled Then tReclamo.SetFocus Else Foco tAclaracion
    End If
End Sub
...
Dim sSerie As String, idPro As Long
    sSerie = InputBox("Ingrese el nº de serie del producto.", "Buscar por nº de serie")
    If sSerie <> "" Then
        Screen.MousePointer = 11
        Cons = "Select ProCodigo, ArtNombre, ProNroSerie From Producto, Articulo " _
            & " Where ProCliente = " & gCliente & " And ProArticulo = ArtID " _
            & " And ProNroSerie = '" & sSerie & "'"
        Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
        If RsAux.EOF Then
            RsAux.Close
...
Private Sub tTelefono_LostFocus()
    If Trim(tTelefono.Text) <> "" Then
        ...
        tTelefono.Tag = clsGeneral.RetornoFormatoTelefono(cBase, tTelefono.Text, Val(tDirCliente.Tag))
...
Dim idServicio As Long
    ...
    If RsAux.EOF Then
        ...
        idServicio = InsertoServicio(vsProducto.Cell(flexcpValue, vsProducto.Row, 0), iEstProd, EstadoS.Taller, cTalDeposito.ItemData(cTalDeposito.ListIndex), Trim(tAclaracion.Text), -1, tUsuario.Tag, Trim(tComentarioInterno.Text))
...
Private Function ValidoIngreso() As Boolean
    ...
    If Trim(tTelefono.Text) <> "" Then
        ...
        RsAux.Close
        tTelefono.Tag = clsGeneral.RetornoFormatoTelefono(cBase, tTelefono.Text, Val(tTelefono.Tag))
...
Private Function InsertoServicio(idProducto As Long, EstadoProducto As Integer, EstadoServicio As Integer, LocalReparacion As Long, Optional Comentario As String = "", Optional LocalRecepcion As Long = -1, Optional Usuario As Long = -1, Optional ComentarioInterno As String = "") As Long
    ...
    Cons = "INSERT INTO Servicio (SerProducto, SerCliente, SerFecha, SerEstadoProducto, SerLocalIngreso, " _
        & " SerLocalReparacion, SerEstadoServicio, SerUsuario, SerModificacion, SerReclamoDe, SerComentario, SerComInterno) Values (" _
...
Dim idServicio As Long, strHora As String
    ...
    If idServicio = 0 Then
        ...
        idServicio = InsertoServicio(vsProducto.Cell(flexcpValue, vsProducto.Row, 0), CInt(vsProducto.Cell(flexcpData, vsProducto.Row, 3)), EstadoS.Retiro, cRDeposito.ItemData(cRDeposito.ListIndex), "", -1, tUsuario.Tag, tComentarioInterno.Text)
        If vsMotivos.Rows > 1 Then InsertoMotivos idServicio
...
Dim idServicio As Long, strHora As String
    ...
    If idServicio = 0 Then
        If Trim(tTelefono.Text) <> "" Then GraboTelefono
        idServicio = InsertoServicio(vsProducto.Cell(flexcpValue, vsProducto.Row, 0), CInt(vsProducto.Cell(flexcpData, vsProducto.Row, 3)), EstadoS.Visita, 0, Trim(tAclaracion.Text), -1, tUsuario.Tag, Trim(tComentarioInterno.Text))
        If vsMotivos.Rows > 1 Then InsertoMotivos idServicio
...
Private Sub ImprimirFichaServicio(ByVal idServicio As Long)
On Error GoTo errFD
Dim iCont As Integer
