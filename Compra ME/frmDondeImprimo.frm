...
Private Sub butOk_Click()
    ...
    If MsgBox("¿Confirma almacenar la configuración?", vbQuestion + vbYesNo, "Grabar") = vbYes Then
        ...
        End If
        oCnfg.GuardarConfiguracion (prmKeyApp)
    End If
    Unload Me
...
Private Sub Form_Load()
    ...
    If (paCodigoDeSucursal = 6) Then
        'Quedamos con Abel en cargar para depósito las de colonia.
        CargoCombo "SELECT TicID, TicNombre From Tickeadora WHERE TicSucursal = " & 5, cboPos, ""
    Else
        CargoCombo "SELECT TicID, TicNombre From Tickeadora WHERE TicSucursal = " & paCodigoDeSucursal, cboPos, ""
    End If
    
    oCnfg.CargarConfiguracion prmKeyApp
