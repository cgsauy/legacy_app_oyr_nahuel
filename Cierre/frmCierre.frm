...
Begin VB.Form frmCierre 
   ...
   Begin VB.Frame fFiltros 
      ...
      Begin VB.Image imgNotas 
         ...
         Stretch         =   -1  'True
         ToolTipText     =   "La terminal tiene pendiente emitir notas sin facturar"
         Top             =   240
         Visible         =   0   'False
         Width           =   360
      End
   ...
   Begin MSComctlLib.ImageList img1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":1430
            Key             =   "salir"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":174A
            Key             =   "help"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":185C
            Key             =   "consultar"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":19B6
            Key             =   "ok"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":1B10
            Key             =   "print"
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":1C6A
            Key             =   "limpiar"
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":1D7C
            Key             =   "error50"
         EndProperty
         BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":21CE
            Key             =   "error100"
         EndProperty
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":2620
            Key             =   ""
         EndProperty
         BeginProperty ListImage10 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCierre.frx":2A72
            Key             =   "error0"
         EndProperty
      EndProperty
   End
...
Dim paMDCuentaRedPagosCamion As Long, paMDCuentaRedPagosClientes As Long
Dim paMDCuentaAbitabCamion As Long, paMDCuentaAbitabClientes As Long
...
Dim bCorregirSaldo As Boolean
    ...
    If Not CargoInformacionDelDia() Then
        MsgBox "Error al refrescar la información, reintente.", vbExclamation, "ATENCIÓN"
        Exit Sub
    End If
    ...
    '5/12/2011 Controlo que no queden moras sin volcar a una nota de débito.
    Cons = "SELECT Top 1 DocCodigo FROM Documento INNER JOIN NotasSinFacturar ON DocCodigo = NSFIDRecibo  And NSFEstado = 0 " _
    ...
    If Not RsAux.EOF Then
        ...
        MsgBox "ATENCIÓN " & vbCrLf & "Existen notas de débitos aún sin facturar, debe hacerlo antes de proceder a realizar el cierre.", vbExclamation, "POSIBLE ERROR"
        Exit Sub
    End If
    RsAux.Close
    ...
    'EJECUTO acumulado artículos.
    '19/01/2017 ya que está trancando el servidor en el job.
    If (1 = mDisponibilidad) Then
        'Lo hago en caja $ colonia que es la última que cierra.
        AcumuladoArticulos
    End If
    Screen.MousePointer = 0
...
Private Function AcumuladoArticulos() As Boolean
On Error GoTo errAA
    cBase.Execute "EXEC batAcumuladoArticulo '" & Format(dFecha.Value, "yyyyMMdd") & "'"
    MsgBox "Se ejecutó el acumulado de ARTICULOS", vbInformation, "ATENCIÓN"
    Exit Function
errAA:
    clsGeneral.OcurrioError "ATENCIÓN!!!" & vbCrLf & vbCrLf & "No se generó el acumulado de artículos, si se hizo el cierre de caja, AVISE A INFORMATICA para que lo ejecute.", Err.Description, "ACUMULADO DE ARTICULOS"
End Function
...
Private Function CargoTransaccionesRedPago(ByVal origen As Integer, ByVal Descripcion As String) As Currency
    
    Cons = "SELECT Count(Distinct(TraID)), IsNull(SUM(comTransaccionItems.TItImporte),0) " & _
        "FROM cgsa.dbo.comTransacciones " & _
        "INNER JOIN cgsa.dbo.comTransaccionItems ON comTransaccionItems.TItTransaccion = TraID " & _
        "WHERE TraPendiente IS NULL AND TraEstado IN(5, 6) AND (TraFUtilizado BETWEEN '" & Format(dFecha.Value, "mm/dd/yyyy") & "' AND '" & Format(dFecha.Value, "mm/dd/yyyy") & " 23:59') " & _
        "AND TraServicio = " & origen '& _
        "GROUP BY TraID"
    
    Dim cant As Integer
    Dim rsRP As rdoResultset
    Set rsRP = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    If Not rsRP.EOF Then
        cant = rsRP(0)
        CargoTransaccionesRedPago = rsRP(1)
    End If
    rsRP.Close
    
    If CargoTransaccionesRedPago <> 0 Then
        vsLista.AddItem Descripcion
        vsLista.Cell(flexcpText, vsLista.Rows - 1, 1) = cant
        If (CargoTransaccionesRedPago) > 0 Then vsLista.Cell(flexcpForeColor, vsLista.Rows - 1, 1, 0, 2) = Colores.Rojo
        vsLista.Cell(flexcpText, vsLista.Rows - 1, 2) = Format(CargoTransaccionesRedPago * -1, "#,##0.00")
        
        vsLista.Cell(flexcpText, vsLista.Rows - 1, 3) = CuentaGiro(origen)
        vsLista.Cell(flexcpText, vsLista.Rows - 1, 4) = 30      'Tipo comprobante salida
    End If
    
End Function

Private Function CuentaGiro(ByVal servicio As Integer) As Long
    Select Case servicio
        Case 0: CuentaGiro = paMDCuentaRedPagosClientes
        Case 2: CuentaGiro = paMDCuentaRedPagosCamion
        Case 5: CuentaGiro = paMDCuentaAbitabClientes
        Case 6: CuentaGiro = paMDCuentaAbitabCamion
    End Select
End Function
...
Private Sub AccionConsultar()
    ...
    End If
    '------------------------------------------------------------------------------------------------------------
    bCierre.Enabled = CargoInformacionDelDia
    Screen.MousePointer = 0
    
End Sub

Private Function CargoInformacionDelDia() As Boolean
Dim aValor As Currency
    ...
    Cons = "SELECT Top 1 DocCodigo FROM Documento INNER JOIN NotasSinFacturar ON DocCodigo = NSFIDRecibo  And NSFEstado = 0 " _
        & "WHERE DocFecha Between '" & Format(dFecha.Value, "mm/dd/yyyy") & " 00:00:00' And '" & Format(dFecha.Value, "mm/dd/yyyy") & " 23:59:59' " _
        & "AND DocSucursal in (" & prmSucursalesDisp & ")"
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    imgNotas.Visible = Not RsAux.EOF
    ...
    Cons = "Select SDiSaldo from SaldoDisponibilidad " _
    ...
    If imgNotas.Visible Then Exit Function
    
    CargoInformacionDelDia = True
    Screen.MousePointer = 0
...
Private Sub Form_Load()
    ...
    Cons = "SELECT Rtrim(ParNombre) Nombre, ParValor FROM Parametro WHERE ParNombre IN ('MCCuentaRedPagosCamioneros', 'MCCuentaRedPagosClientes', 'MCCuentaAbitabCamioneros', 'MCCuentaAbitabClientes')"
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    Do While Not RsAux.EOF
    
        Select Case LCase(RsAux("Nombre"))
            Case LCase("MCCuentaRedPagosClientes"): paMDCuentaRedPagosClientes = RsAux("ParValor")
            Case LCase("MCCuentaRedPagosCamioneros"): paMDCuentaRedPagosCamion = RsAux("ParValor")
            Case LCase("MCCuentaAbitabCamioneros"): paMDCuentaAbitabCamion = RsAux("ParValor")
            Case LCase("MCCuentaAbitabClientes"): paMDCuentaAbitabClientes = RsAux("ParValor")
        End Select
        RsAux.MoveNext
    Loop
    RsAux.Close
    
    If (paMDCuentaAbitabCamion = 0 Or paMDCuentaAbitabClientes = 0 Or paMDCuentaRedPagosClientes = 0) Then
        MsgBox "Atención los parámetros de cuentas de giros no son correctos.", vbExclamation, "ATENCIÓN"
    End If
    ...
    If paMDCuentaRedPagosCamion = 0 Then
        MsgBox "No se cargo la cuenta de redpagos camioneros.", vbInformation, "ATENCIÓN"
    End If
    ...
    sFechas = "'20140701:" & Format(Date - 1, "yyyyMMdd") & "'"
    Cons = "SELECT FFecha FROM CGSA.dbo.TablaFecha(" & sFechas & ") WHERE FFecha NOT IN (SELECT erdFecha FRom eReportesDiarios WHERE ERDEstado = 1)"
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    sFechas = vbNullString
    Do While Not RsAux.EOF
        sFechas = sFechas & IIf(sFechas <> vbNullString, ", ", "") & Format(RsAux("fFecha"), "dd/MM/yyyy")
        RsAux.MoveNext
    Loop
    RsAux.Close
    If sFechas <> "" Then
        MsgBox "El reporte diario a enviar a DGI no fue generado para la(s) siguiente(s) fecha(s): " & sFechas, vbExclamation, "ATENCIÓN"
    End If
End Sub
...
Dim aCantidadC As Long, aSumaC As Currency
    ...
    If CStr(dFecha.Value) = Format(gFechaServidor, "dd/MM/yyyy") Then
        ...
        '31/7/2014 A partir de ahora las notas de débitos anuladas son aquellas que tienen una nota de crédito/devolución relacionada al recibo.
        Set RsAux = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurValues)
        If Not RsAux.EOF Then
        ...
        RsAux.Close
    Else
        Dim oMoras As clsCantidadImporte
        Set oMoras = modZureo.CGSA_CobranzaMorasAFecha(dFecha.Value, idsSucursales)
        .Cell(flexcpText, RowMora, 1) = oMoras.Cantidad
        .Cell(flexcpText, RowMora, 2) = Format(oMoras.Importe, "#,##0.00")
    End If
...
Private Sub CargoEgresosContado(Row As Integer, idsSucursales As String, Moneda As Long)
    ...
    Cons = Cons & " And DocTipo = " & TipoDocumento.NotaDevolucion _
                        & " And DocMoneda = " & Moneda _
                        & " And DocAnulado = 0 AND DocCodigo NOT IN (SELECT DPaDocASaldar FROM DocumentoPago WHERE DPaMora <> 0)"
...
'    cons = cons & " And MDiTipo = TMDCodigo" _
    ...
    If Disponibilidad = 1 Then
        CargoTransaccionesRedPago 2, "Redpagos camioneros"
        CargoTransaccionesRedPago 0, "Redpagos clientes"
        
        CargoTransaccionesRedPago 6, "Abitab camioneros"
        CargoTransaccionesRedPago 5, "Abitab clientes"
    End If
...
Private Sub MnuPMovPendienteCamion_Click()
    EjecutarApp prmPathApp & "\appExploreMsg.exe ", "568:" & dFecha.Value
End Sub
...
'                " And (DPeIdLiquidacion is Null OR DPeFLiquidacion > " & Format(mFecha, "'mm/dd/yyyy 23:59:59'") & ")"
    ...
    Cons = "SELECT IsNull(Sum(ISNULL(DPeImporte, DocTotal)), 0) + " & _
        "IsNull((SELECT Sum(ISNULL(DPeImporte, DocTotal)) " & _
        "FROM cgsa.dbo.Documento INNER JOIN cgsa.dbo.DocumentoPendiente ON DocCodigo = DPeDocumento And DPeDisponibilidad = " & mDisponibilidad & " And (DPeIdLiquidacion is Null OR DPeFLiquidacion > '" & Format(CDate(mFecha), "mm/dd/yyyy 23:59:59") & "') " & _
        "INNER JOIN(SELECT CONVERT(int, Substring(DocComentario, CharIndex('(', DocComentario)+1, CharIndex(')', DocComentario) - LEN(' Cambio nombre doc ('))) IDDoc, DocFecha FechaOrig " & _
        "FROM cgsa.dbo.Documento WHERE DocTipo = 1 AND DocAnulado = 0 AND DocSucursal = 6 AND CharIndex('Cambio nombre doc (', DocComentario) > 0 And DocFecha < '" & Format(CDate(mFecha), "mm/dd/yyyy 23:59:59") & "') AS CNom ON IDDoc = DocCodigo AND FechaOrig < DPeFLiquidacion " & _
        "WHERE DocAnulado = 0 AND DocFecha > '" & Format(CDate(mFecha), "mm/dd/yyyy 23:59:59") & "'), 0) Total " & _
        "FROM cgsa.dbo.DocumentoPendiente LEFT Outer Join cgsa.dbo.Liquidacion On DPeIDLiquidacion = LiqID, CGSA.dbo.Documento " & _
        "Where DPeDocumento = DocCodigo And DocAnulado = 0 " & _
        "And DocFecha <= '" & Format(CDate(mFecha), "mm/dd/yyyy 23:59:59") & "' And DPeDisponibilidad = " & mDisponibilidad & " And (DPeIdLiquidacion is Null OR DPeFLiquidacion > '" & Format(CDate(mFecha), "mm/dd/yyyy 23:59:59") & "')"
