...
'    Cons = " Select NomCli = RTrim(CPeApellido1) + RTrim(' ' + CPeApellido2)+', ' + RTrim(CPeNombre1)+ " _
'                & " RTrim(' ' + CPeNombre2), NomEmp=(RTrim(CEmFantasia) + RTrim(' (' + CEmNombre)+ ')'), " _
'                & " DocCodigo, DocAnulado, DocPendiente, DocSerie, DocNumero, DocFModificacion, SerCodigo, SerEstadoServicio, EAUFechaHora, EAuTiempoProm, DocFRetira " _
'                & ", case DatePart(hh, DocFRetira) When 1 Then DocFRetira-DatePart(n, DocFRetira) Else DocFecha End as Desde" _
'           & " From (((((Documento Inner Join Cliente ON Documento.DocCliente = Cliente.CliCodigo) " _
'           & " Left Outer Join Cpersona ON Cliente.CliCodigo = Cpersona.CpeCliente) " _
'           & " Left Outer Join CEmpresa ON CliCodigo = CEmCliente) " _
'           & " Left Outer Join Servicio ON SerDocumento = DocCodigo) " _
'           & " Left Outer Join EntregaAuxiliar ON DocTipo = EAuTipo And DocCodigo = EAuDocumento And EAuEstado In (1,2))" _
'           & " Where DocCodigo = " & Codigo & " And DocTipo = " & iTipo
'
'    Set RsAux = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurValues)
'
'    If Not RsAux.EOF Then
...
'            loc_ShowMsg "Documento pendiente de entrega, consulte en mostrador.", 7000, Advierto
'            Exit Sub
'        ElseIf Not IsNull(RsAux("SerCodigo")) Then
'            Screen.MousePointer = 0
'            Select Case RsAux("SerEstadoServicio")
'                Case EstadoS.Cumplido, EstadoS.Anulado
'                    RsAux.Close
'                    loc_ShowMsg "Servicio " & iAux & ", consulte en mostrador por el estado de su servicio.", 6000, Advierto
'                Case EstadoS.Entrega
'                    RsAux.Close
'                    loc_ShowMsg "El servicio " & iAux & ", esta pendiente de entrega en domicilio, consulte en mostrador.", 6000, Advierto
'                Case Else
'                    iAux = RsAux("SerCodigo")
'
'                    iDocumento = RsAux!DocCodigo
...
'    Else
'        RsAux.Close
'        loc_ShowMsg "No existe un documento para las características ingresadas, consulte en mostrador.", 7000, Advierto
'        Exit Sub
'    End If
'
'
...
'
'        If Not (Date > Format(RsAux("desde"), "dd/mm/yyyy") And Date <= Format(RsAux("DocFRetira"), "dd/mm/yyyy")) Then
'
'       ' If CDate(Format(RsAux("DocFRetira"), "dd/mm/yyyy")) < Date Then
'            RsAux.Close
'            loc_ShowMsg "No existe un documento para las características ingresadas, consulte en mostrador.", 7000, Advierto
'            Exit Sub
''        Else
''            If CDate(Format(RsAux("DocFRetira"), "dd/mm/yyyy")) > Date And CInt(Format(RsAux("DocFRetira"), "hh")) = 1 Then
''                RsAux.Close
''                loc_ShowMsg "No existe un documento para las características ingresadas, consulte en mostrador.", 7000, Advierto
''                Exit Sub
''            End If
  ...
  '          Else
   ...
   '         End If
    ...
    ElseIf vsGrid2.Rows > 0 Then
        ...
        If bLocalAlternativo Then
            loc_ShowMsg "Atención en el local «" & sLocalAlternativo & "» puede retirar toda la mercadería.", 9000, Advierto
