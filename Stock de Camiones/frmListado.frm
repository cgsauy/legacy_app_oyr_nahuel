...
Begin VB.Form frmListado 
   Caption         =   "Veo Stock Servicio por Cliente"
   ClientHeight    =   6810
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   10365
...
Dim aValor As Long
  On Error GoTo ErrArmoGrilla
    With vsConsulta
        ...
        .FormatString = "Artículo|<Camion|<Estado|>Q Tabla|>Servicios|>Traslado|>Envio|"
        .ColWidth(0) = 2800: .ColWidth(1) = 1100: .ColWidth(2) = 1100
        .MergeCol(0) = True
        .Redraw = True
    End With
    Screen.MousePointer = 0
...
Dim RsCam As rdoResultset, RsEstado As rdoResultset
    ...
    With vsConsulta
        ...
        Set RsEstado = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
        If RsEstado.EOF Then
            RsEstado.Close
            MsgBox "No hay estados físicos ingresados.", vbExclamation, "ATENCIÓN"
            Exit Sub
        End If
        ...
        Do While Not RsAux.EOF
            ...
            idArticulo = 0: .Cell(flexcpData, .Rows - 1, 3) = idArticulo
            
            'Para el artículo levanto el stock de cada camión.
            Cons = "Select CamCodigo, CamNombre From Camion "
            If cCamion.ListIndex > -1 Then Cons = Cons & " Where CamCodigo = " & cCamion.ItemData(cCamion.ListIndex)
            Set RsCam = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurValues)
            
            Do While Not RsCam.EOF
                'Ahora para cada estado me fijo si tiene stock y si necesita.
                RsEstado.MoveFirst
                Do While Not RsEstado.EOF
                    .Cell(flexcpText, .Rows - 1, 1) = Trim(RsCam!CamNombre)
                    idArticulo = RsCam!CamCodigo: .Cell(flexcpData, .Rows - 1, 1) = idArticulo
                    .Cell(flexcpText, .Rows - 1, 2) = Trim(RsEstado!EsMAbreviacion)
                    idArticulo = RsEstado!EsMCodigo: .Cell(flexcpData, .Rows - 1, 2) = idArticulo
                    
                    'Veo si hay datos en la tabla stocklocal.
                    .Cell(flexcpText, .Rows - 1, 3) = StockTablaLocal(RsAux!ArtID, RsCam!CamCodigo, RsEstado!EsMCodigo)
                    
                    'Para los servicios solo consulto el estado a Recuperar.
                    If RsEstado!EsMCodigo = paEstadoARecuperar Then
                        .Cell(flexcpText, .Rows - 1, 4) = StockEnServicios(RsAux!ArtID, RsCam!CamCodigo)
                    Else
...
Private Function StockTablaLocal(Articulo As Long, Camion As Long, Estado As Integer)
    StockTablaLocal = 0
    Cons = "Select * From StockLocal " _
        & " Where StLArticulo = " & Articulo & " And StLTipoLocal = " & TipoLocal.Camion _
        & " And StLLocal = " & Camion & " And StLEstado = " & Estado
    Set RsSt = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurReadOnly)
    If Not RsSt.EOF Then StockTablaLocal = RsSt!StLCantidad
    RsSt.Close
End Function

Private Function StockEnServicios(Articulo As Long, Camion As Long)
    StockEnServicios = 0
    Cons = "Select Count(*) From TrasladoServicio, Producto , Taller, Servicio " _
        & " Where TSeProducto = ProCodigo And ProCliente = " & paClienteEmpresa _
        & " And ProArticulo = " & Articulo & " And TalIngresoCamion = " & Camion _
        & " And SerProducto = ProCodigo and SerCodigo = TalServicio" _
        & "And ((TalFIngresorealizado is not null and talfingresorecepcion is null) or (talfsalidarealizado is not null and talfsalidarecepcion is null))"
    Set RsSt = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurReadOnly)
    If Not RsSt.EOF Then StockEnServicios = RsSt(0)
    RsSt.Close
End Function
...
On Error GoTo ErrSAFE
    ...
    'Saco la cantidad que tiene sin entregar.
    Cons = "Select Sum(RTrPendiente) From Traspaso, RenglonTraspaso" _
        & " Where TraLocalIntermedio = " & Camion _
        & " And TraFechaEntregado Is Null And TraFImpreso Is Not Null " _
        & " And RTrArticulo = " & Articulo & " And RTrEstado = " & IDEstado _
        & "And TraCodigo = RTrTraspaso"
    Set RsSt = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurReadOnly)
    If IsNull(RsSt(0)) Then StockEnTrasladoXEstado = 0 Else StockEnTrasladoXEstado = RsSt(0)
    RsSt.Close
    
    'Saco la diferencia de lo que no entrego.
    Cons = "Select Sum(RTrPendiente) From Traspaso, RenglonTraspaso" _
        & " Where TraLocalIntermedio = " & Camion _
        & " And TraFechaEntregado Is Not Null And TraFImpreso Is Not Null " _
        & " And RTrArticulo = " & Articulo & " And RTrEstado = " & IDEstado _
        & "And TraCodigo = RTrTraspaso"
    Set RsSt = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurReadOnly)
