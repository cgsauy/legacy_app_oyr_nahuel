...
Private Sub cDisponibilidad_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If Not IsNumeric(tImporte.Text) Then
            MsgBox "El importe ingresado no es correcto. Verifique", vbExclamation, "Posible Error"
            Foco tImporteP: Exit Sub
        End If
        If cDisponibilidad.ListIndex = -1 Then
            MsgBox "Debe seleccionar la disponibilidad con la que se realizará el pago.", vbExclamation, "Posible Error"
            Foco cDisponibilidad: Exit Sub
        End If
        
        Dim mIDDisp As Long, mMonedaD As Long, mBancaria As Boolean
        mIDDisp = cDisponibilidad.ItemData(cDisponibilidad.ListIndex)
        
        'Consulta para ver que tipo de disponibilidad es-----------------------------------------------------------
        cons = "Select * from Disponibilidad Where DisID = " & mIDDisp
        Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
        mMonedaD = rsAux!DisMoneda
        mBancaria = Not IsNull(rsAux!DisSucursal)
        rsAux.Close
        '------------------------------------------------------------------------------------------------------------------
        
        If mMonedaD <> cMoneda.ItemData(cMoneda.ListIndex) Then
            tImporteD.Enabled = True: tImporteD.BackColor = Obligatorio
            If mMonedaD = paMonedaPesos And IsNumeric(tPesos.Text) Then tImporteD.Text = tPesos.Text
        Else
            tImporteD.Text = tImporteP.Text
        End If
        
        If mBancaria Then
            orCheque.Enabled = True: orCheque.BackColor = cDisponibilidad.BackColor
            With orCheque
                .prp_IdDisponibilidad = mIDDisp
                .prp_IdGasto = tIDCompra.Tag
                If IsNumeric(tImporteD.Text) Then .prp_ValorAAsignar = CCur(tImporteD.Text)
        ...
        If orCheque.Enabled Then orCheque.SetFocus: Exit Sub
...
Dim aImporte As Currency
    ...
    On Error Resume Next
    If Me.Visible Then CargoCamposDesdeBD tIDCompra.Tag
    ...
    Foco tImporteP
    lPagaX.Caption = "Pa&ga " & Trim(cMoneda.Text) & ":"
...
Dim aSuma, aImporte As Currency
        ...
        For I = 1 To .Rows - 1      '---------------------------------------------------------------------------
            ...
            If .Cell(flexcpData, I, 1) = cDisponibilidad.ItemData(cDisponibilidad.ListIndex) Then
                If Trim(.Cell(flexcpText, I, 3)) = "" Then
                    MsgBox "La disponibilidad ya está ingersada. No podrá ingresar doble registro.", vbExclamation, "Disponibilidad Ingresada"
                    Exit Sub
                End If
                'Veo si es el mismo cheque
                If orCheque.fnc_GetValorData("") Then
                    If Trim(.Cell(flexcpText, .Rows - 1, 3)) = Trim(orCheque.fnc_GetValorData("CheSerie")) & " " & Trim(orCheque.fnc_GetValorData("CheNumero")) Then
                        MsgBox "El comprobante de pago ya está ingresado. " & vbCrLf & _
                                    "No podrá ingresar doble registro.", vbExclamation, "Comprobante Ingresado"
                        Exit Sub
                    End If
                End If
            End If
    ...
    If orCheque.fnc_GetValorData("CheID") <> 0 Then
        Dim rsCh As rdoResultset
        
        Dim aValorCheque As Currency
        aValorCheque = orCheque.fnc_GetValorData("CheImporte")
        ...
        cons = "Select Sum(CPaImporte) from ChequePago " & _
                    " Where CPaIdCheque = " & orCheque.fnc_GetValorData("CheID") & _
                    " And CPaIDCompra <> " & Val(tIDCompra.Tag)
        Set rsCh = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
        If Not rsCh.EOF Then
            If Not IsNull(rsCh(0)) Then
                If rsCh(0) + aValorQMuevo > aValorCheque Then
                    MsgBox "El importe ingresado supera el saldo disponible en el cheque." & vbCrLf & vbCrLf _
                            & "Importe Original del Cheque: " & Format(aValorCheque, "#,##0.00") & vbCrLf _
                            & "Importe Disponible para Asignar: " & Format(rsCh(0) - aValorCheque, "#,##0.00"), vbExclamation, "Importe Incorrecto."
    ...
    With vsPago
        .AddItem Format(tImporteP.Text, "#,##0.00")
        ...
        .Cell(flexcpText, .Rows - 1, 2) = tImporteD.Text
        
        If orCheque.fnc_GetValorData("") Then
            .Cell(flexcpText, .Rows - 1, 3) = Trim(orCheque.fnc_GetValorData("CheSerie")) & " " & Trim(orCheque.fnc_GetValorData("CheNumero"))
            aValor = orCheque.fnc_GetValorData("CheID")
            .Cell(flexcpData, .Rows - 1, 3) = aValor
    
            .Cell(flexcpText, .Rows - 1, 4) = Format(orCheque.fnc_GetValorData("CheImporte"), "#,##0.00")
            .Cell(flexcpText, .Rows - 1, 5) = Format(orCheque.fnc_GetValorData("CheLibrado"), "dd/mm/yy")
            .Cell(flexcpText, .Rows - 1, 6) = Format(orCheque.fnc_GetValorData("CheVencimiento"), "dd/mm/yy")
        End If
        
         .Cell(flexcpText, .Rows - 1, 7) = Format(tPesos.Text, "#,##0.00")  'Importe en pesos
    End With
    
    cDisponibilidad.Text = "": tPesos.Text = "": tImporteD.Text = ""
...
Dim aMsgErr As String
    ...
    If gFModificacion <> RsCom!ComFModificacion Then
        aMsgErr = "La ficha de compra ha sido modificada desde otra terminal. " & vbCrLf & _
                        "Para grabar vuelva a cargar los datos."
...
Dim RsMov As rdoResultset
    ...
    cons = "Select * from MovimientoDisponibilidad Where MDiIDCompra = " & Compra
    Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
    If Not rsAux.EOF Then aIDMovimiento = rsAux!MDiID
    
    If aIDMovimiento = 0 Then
        If rsAux.EOF Then rsAux.AddNew Else rsAux.Edit
        rsAux!MDiFecha = Format(tFecha.Text, "mm/dd/yyyy")
    ...
    If sNuevo Then
        cons = "Select Max(MDiID) from MovimientoDisponibilidad" & _
                    " Where MDiFecha = " & Format(tFecha.Text, "'mm/dd/yyyy'") & _
                    " And MDiHora = " & Format(gFechaServidor, "'hh:mm:ss'") & _
                    " And MDiTipo = " & paMDPagoDeCompra & _
                    " And MDiIdCompra = " & Compra
        Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
    ...
    If sModificar Then
        Dim bDel As Boolean
        'Borro los movimientos de las Disponiblidades que no estaban CERRADAS
        ...
        Do While Not RsMov.EOF
            ...
            With vsPago
                For I = 1 To .Rows - 1
                    If (RsMov!MDRIdDisponibilidad = .Cell(flexcpData, I, 1)) Then
                        If (Trim(.Cell(flexcpText, I, 8)) <> "") Then bDel = False
                        Exit For
                    End If
                Next
            End With
            
            'O no está en la grilla o está y no estaba cerrada
            If bDel Then
                If RsMov!MDRIdCheque Then dis_BorroRelacionCheque RsMov!MDRIdCheque, Compra
                RsMov.Delete
            End If
    ...
    With vsPago
    For I = 1 To .Rows - 1
        If Trim(.Cell(flexcpText, I, 8)) = "" Then
            RsMov.AddNew
            RsMov!MDRIdMovimiento = aIDMovimiento
            RsMov!MDRIdDisponibilidad = .Cell(flexcpData, I, 1)
        
            aIDCheque = 0
            If Trim(.Cell(flexcpText, I, 3)) <> "" Then 'Se paga con un cheque----------------------------------------------
                
                If .Cell(flexcpData, I, 3) <> 0 Then aIDCheque = .Cell(flexcpData, I, 3)    'El cheque ya existe
                
                'Inserto el Cheque
                cons = "Select * from Cheque Where CheID = " & aIDCheque
                Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
                If rsAux.EOF Then
                    rsAux.AddNew
                    ...
                    cons = "Select Max(CheID) from Cheque" & _
                            " Where CheIDDisponibilidad = " & .Cell(flexcpData, I, 1)
                    Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
                    aIDCheque = rsAux(0)
                End If
                rsAux.Close
...
Private Sub AccionEliminar()
    ...
    If Not rsAux.EOF Then
        ...
        Do While Not RsRen.EOF
            dis_BorroRelacionCheque RsRen!MDRIdCheque, tIDCompra.Tag
            RsRen.MoveNext
        Loop
        RsRen.Close
...
Private Sub orCheque_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then AgregoPago
End Sub
...
Private Sub tImporteP_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        ...
        If cMoneda.ItemData(cMoneda.ListIndex) <> paMonedaPesos Then
            ...
            lTC.Caption = "TC Dólar: " & Format(tPesos.Tag, "#,##0.000")
        Else
            tPesos.Text = tImporteP.Text
        End If
...
Private Sub DeshabilitoIngreso()
    ...
    cComprobante.Enabled = True: cComprobante.BackColor = Blanco
    ...
    orCheque.Enabled = False: orCheque.BackColor = Inactivo
...
Dim rs1 As rdoResultset
    ...
    If RsCom.EOF Then
        MsgBox "No existe una compra para el código ingresado." & vbCrLf & _
                    "(*) Contado, nota de devolución, nota de crédito, recibo de pago, recibo provisorio, ingreso o salida de caja.", vbExclamation, "ATENCIÓN"
        Screen.MousePointer = 0
    ...
    tFecha.Text = Format(RsCom!ComFecha, "dd/mm/yyyy")
    ...
    If Not rsAux.EOF Then
        ...
        Do While Not rsAux.EOF
            ...
            Else
                .Cell(flexcpText, .Rows - 1, 2) = Format(rsAux!MDRDebe, FormatoMonedaP)
            End If
            ...
            If Not IsNull(rsAux!DisSucursal) Then       'Si la Disponibilidad es bancaria
                ...
                If Not rs1.EOF Then
                    .Cell(flexcpText, .Rows - 1, 3) = Trim(rs1!CheSerie) & " " & rs1!CheNumero
                     ...
                     .Cell(flexcpText, .Rows - 1, 5) = Format(rs1!CheLibrado, "dd/mm/yy")
                     If Not IsNull(rs1!CheVencimiento) Then .Cell(flexcpText, .Rows - 1, 6) = Format(rs1!CheVencimiento, "dd/mm/yy")
                End If
                rs1.Close
            End If
            ...
            'Valido si la disponibilidad está CERRADA   -----------------------------------------------------------------
            Dim mDate As Date, mDateG As Date
            Dim bCerrada As Boolean, mIdx As Integer
            bCerrada = False
            mDate = dis_FechaCierre(rsAux!DisID, CDate(rsAux!MDiFecha))
            mDateG = rsAux!MDiFecha
            
            If Trim(.Cell(flexcpText, .Rows - 1, 3)) <> "" Then
                If Trim(.Cell(flexcpText, .Rows - 1, 6)) <> "" Then
                    mDateG = CDate(.Cell(flexcpText, .Rows - 1, 6))
                Else
                    mDateG = CDate(.Cell(flexcpText, .Rows - 1, 5))
                End If
            End If
            If mDate >= mDateG Then bCerrada = True
            
            If bCerrada Then
                .Cell(flexcpBackColor, .Rows - 1, 0, , .Cols - 1) = Colores.GrisOscuro
                .Cell(flexcpText, .Rows - 1, 8) = "1"
            End If
            '--------------------------------------------------------------------------------------------------------------------
            rsAux.MoveNext
        Loop
...
Private Sub InicializoGrillas()
    ...
    With vsPago
        .Rows = 1: .Cols = 1
        .Editable = False
        .FormatString = ">Importe|Disponibilidad|>$ Disponibilidad|<Nº Cheque|>Importe|Librado|Vence|Pesos|Disp.Cerrada"
            
        .WordWrap = False
        .ColHidden(7) = True: .ColHidden(8) = True
        .ColWidth(0) = 1100: .ColWidth(1) = 1900: .ColWidth(2) = 1200: .ColWidth(4) = 1100: .ColWidth(5) = 800
...
Private Sub tImporteD_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        If cDisponibilidad.ListIndex = -1 Then Foco cDisponibilidad: Exit Sub
        If Not IsNumeric(tImporteD.Text) Then Exit Sub
        If IsNumeric(tImporteD.Text) Then tImporteD.Text = Format(tImporteD.Text, FormatoMonedaP)
        
        With orCheque
            .prp_IdDisponibilidad = cDisponibilidad.ItemData(cDisponibilidad.ListIndex)
            .prp_IdGasto = tIDCompra.Tag
            .prp_ValorAAsignar = CCur(tImporteD.Text)
            .prp_IdCheque = Val(orCheque.Tag)
            .prp_ValorInicial = 1
            .fnc_Show
        End With
...
Private Sub vsPago_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyDelete Then
        If vsPago.Row > 0 Then
            If Trim(vsPago.Cell(flexcpText, vsPago.Row, 8)) <> "" Then Exit Sub
            vsPago.RemoveItem vsPago.Row
            If vsPago.Rows = 1 Then Foco tImporteP
        End If
    End If
...
Private Sub DeshabilitoCamposCheque(Optional DesdeDisponibilidad As Boolean = False, Optional DesdeTipo As Boolean = False)
    ...
    If Trim(tImporteD.Text) <> "" Then tImporteD.Text = ""
    If tImporteD.Enabled Then
        tImporteD.Enabled = False: tImporteD.BackColor = Colores.Inactivo
    End If
        
    If orCheque.Enabled Then
        orCheque.fnc_BlankControls
...
Private Function ValidoCamposPago() As Boolean
    ...
    End If
    If Not IsNumeric(tPesos.Text) Then Foco tImporteP: Exit Function
    If Not IsNumeric(tImporteD.Text) Then Foco cDisponibilidad: Exit Function
    ...
    'VAlidar cierre con fechas del cheque
    Dim mDate As Date, mDateG As Date
    Dim bCerrada As Boolean, mIdx As Integer
    bCerrada = False
    ...
    If bCerrada Then
        MsgBox "La disponibilidad " & cDisponibilidad.Text & " está cerrada." & vbCrLf & _
                    "No puede usar ésta disponibilidad para ingresar el pago", vbExclamation, "Disponibilidad Cerrada"
        Foco cDisponibilidad
        Exit Function
    End If
...
Private Function PidoSucesoEliminar() As Boolean
    ...
    'Valido si alguna de las disponibilidades fue cerrada       ----------------------------
    With vsPago
        For I = 1 To .Rows - 1
            If Trim(.Cell(flexcpText, I, 8)) <> "" Then
                MsgBox "La disponiblidad " & .Cell(flexcpText, I, 1) & " al " & Trim(tFecha.Text) & " está cerrada." & vbCrLf & _
                            "No se puede eliminar el pago.", vbExclamation, "Disponibilidad Cerrada"
                Exit Function
            End If
        Next
...
errSuceso:
    clsGeneral.OcurrioError "Error al validar datos para eliminar.", Err.Description
    Screen.MousePointer = 0
End Function
