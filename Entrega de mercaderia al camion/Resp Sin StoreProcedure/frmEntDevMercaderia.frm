        ...
        For iQ = .FixedRows To .Rows - 1
            If .Cell(flexcpValue, iQ, 0) > 0 Then
                ...
                If CDate(.Cell(flexcpData, iQ, 1)) <> rsRE("ReEFModificacion") Then
                    sErr = "Código de impresión modificado por otra terminal, tiene que volver a cargar toda la información."
                    rsRE.Close
                End If
                rsRE.Edit
                ...
                If prm_Tipo = 1 Then
                    ...
                    If sSuceso <> "" Then
                        objGral.RegistroSuceso cBase, gFechaServidor, TipoSuceso.DiferenciaDeArticulos, paCodigoDeTerminal, CLng(tUID.Tag), 0, CLng(.Cell(flexcpData, iQ, 0)), _
                                Descripcion:="Entrega de Mercadería al Camión, código: " & tCodigo.Text, Defensa:="Entrega " & CInt(.Cell(flexcpValue, iQ, 0)) & " art(s). " & Mid(Trim(.Cell(flexcpText, iQ, 3)), 1, 15) & "... " & sSuceso
                    End If
