...
Private Sub InicializoGrillas()
    ...
    With vsConsulta
        ...
        .Cols = 1: .Rows = 1:
        .FormatString = "<Artículo|>Se Contó|>Existencia|>Diferencia|>Q Rotos|>Valorizada|"
...
Private Sub AccionConsultar()
    ...
    Cons = "Select Count(Distinct(VenArticulo)) From CMVenta"
    ...
    Cons = "Select Count(Distinct(CCBArticulo)) from ContadoCalcBalance" & _
                " Where CCBArticulo Not In (Select VenArticulo from CMVenta)"
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    If Not RsAux.EOF Then If Not IsNull(RsAux(0)) Then aQ = aQ + RsAux(0)
    RsAux.Close
    ...
    With vsConsulta
        ...
        Cons = "Select  ArtID, ArtCodigo, ArtNombre, Sum(VenCantidad) as Q" & _
        ...
        Do While Not RsAux.EOF
            ...
            .Cell(flexcpText, .Rows - 1, 2) = Format(RsAux!Q, "#,##0")
            ...
            If QSano <> 0 Then .Cell(flexcpText, .Rows - 1, 1) = Format(QSano, "#,##0")
            If QRoto <> 0 Then .Cell(flexcpText, .Rows - 1, 4) = Format(QRoto, "#,##0")
            
            .Cell(flexcpText, .Rows - 1, 3) = Format(.Cell(flexcpValue, .Rows - 1, 2) - .Cell(flexcpValue, .Rows - 1, 1), "#,##0")
        ...
        Cons = "Select ArtID, ArtCodigo, ArtNombre, Sum(CCBCantidad) as Q" & _
                   ...
                   " Where CCBArticulo Not In (Select VenArticulo from CMVenta)" & _
...
Private Sub CargoTablaCMCosteo(idArticulo As Long, Cantidad As Long)
        ...
        Do While aQVenta <> 0
                ...
                If Not rsCom.EOF Then               'Hay una FC <= FV
                    If aQVenta > 0 Then                 'VENTA DE MERCADERIA---------------------------------------------------
                        ...
                        Total = Total + (aQCosteo * rsCom!ComCosto)
