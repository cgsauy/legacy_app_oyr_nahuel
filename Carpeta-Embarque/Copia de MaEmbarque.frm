...
Begin VB.Form MaEmbarque 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.TextBox tConocimiento 
         ...
         Width           =   2175
      End
      Begin VB.TextBox tCarpetaDespachante 
         Appearance      =   0  'Flat
         Height          =   285
         Left            =   1080
         MaxLength       =   10
...
Private Sub cAgencia_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then Foco cMTransporte
End Sub
...
Private Sub cDestino_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then Foco tCarpetaDespachante
End Sub
    ...
    If KeyAscii = vbKeyReturn Then
        ...
        If cTransporte.ListIndex > -1 And vsContenedor.Rows > 1 Then
            ...
            If Not RsAux.EOF Then
                If Not IsNull(RsAux!TraLinea) Then idLinea = RsAux!TraLinea
            End If
            If idLinea > 0 Then
...
Private Sub tArriboPrevisto_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then Foco tUltFechaEmbarque
End Sub
...
Private Sub HabilitoCamposEmbarque()
    ...
    tCarpetaDespachante.Enabled = True: tCarpetaDespachante.BackColor = vbWhite
...
On Error GoTo ErrCDE
    ...
    If Not IsNull(RsEmbarque!EmbTransporte) Then BuscoCodigoEnCombo cTransporte, RsEmbarque!EmbTransporte
    If Not IsNull(RsEmbarque!EmbCarpetaDesp) Then tCarpetaDespachante.Text = Trim(RsEmbarque!EmbCarpetaDesp)
    If Not IsNull(RsEmbarque!EmbFlete) Then tFlete.Text = Format(RsEmbarque!EmbFlete, FormatoMonedaP)
    ...
    If Not IsNull(RsEmbarque!EmbFEmbarque) Then tEmbarco.Text = Format(RsEmbarque!EmbFEmbarque, FormatoFP)
    If Not IsNull(RsEmbarque!EmbFAPrometido) Then tArriboPrevisto.Text = Format(RsEmbarque!EmbFAPrometido, FormatoFP)
    If Not IsNull(RsEmbarque!EmbFArribo) Then labArribo.Caption = Format(RsEmbarque!EmbFArribo, FormatoFP)
...
Dim LetraEmb As String * 1
    ...
    Dim iBcoEm As Long, iProv As Long
...
Private Function fnc_GetImporteGastos(ByVal idEmb As Long, ByVal iImpGasto As Double) As Boolean
...
'    FechaDelServidor
    ...
    Else
        ...
        Cons = "Select * From Embarque Where EmbID = " _
            ...
            & " And EmbFEmbarque = Null And EmbCodigo > '" & LetraEmb & "')"
        Set RsAuxE = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
        If RsAuxE.EOF Then
            RsAuxE.Close
            Cons = "Select Max(EmbCodigo) From Embarque Where EmbCarpeta = " & tCodigo.Tag
            ...
            'Si la divisa esta paga tengo que mantener el valor de la misma.
            If sDivisaPaga Then
...
errPasoGastos:
    clsGeneral.OcurrioError "Error al pasar los gastos a zureo.", Err.Description, "Embarque"
    GoTo Fin
        ...
        If Valor > 0 Then
            ...
            If Not RsAuxE.EOF Then
                ...
                If Not sDivisaPaga Then
                    ...
                    aAux = ModCarpeta.InsertoGastoImportacionZureo(idEmbarque, Valor, Format(gFechaServidor, FormatoFP), RsAuxE!ComProveedor, RsAuxE!CarCodigo & "." & RsAuxE("EmbCodigo"), RsAuxE("EmbCodigo"), RsAuxE!ComSerie & " " & RsAuxE!ComNumero, TipoDocumento.CompraCredito, RsAuxE!EmbArbitraje, labBcoEmisor.Caption, labLC.Caption, labFormaPago.Caption, paSubrubroDivisa, iBcoEm)
                Else
                    aAux = ModCarpeta.InsertoGastoImportacionZureo(idEmbarque, Valor, Format(gFechaServidor, FormatoFP), RsAuxE!ComProveedor, RsAuxE!CarCodigo & "." & RsAuxE("EmbCodigo"), RsAuxE("EmbCodigo"), RsAuxE!ComSerie & " " & RsAuxE!ComNumero, TipoDocumento.CompraCredito, RsAuxE!EmbArbitraje, labBcoEmisor.Caption, labLC.Caption, labFormaPago.Caption, paSubrubroDivisa, True)
                End If
...
estoy en esta mierda
    ...
    If Not RsAuxE.EOF Then
        ...
        Dim iBcoEm As Integer
        If Not IsNull(RsAuxE!CarBcoEmisor) Then iBcoEm = RsAuxE("CarBcoEmisor") Else iBcoEm = 0
        IDNota = ModCarpeta.InsertoGastoImportacionZureo(idEmbarque, aValor * -1, Format(gFechaServidor, FormatoFP), RsAuxE!ComProveedor, RsAuxE!CarCodigo & "." & RsAuxE!EmbCodigo, RsAuxE!EmbCodigo, Trim(RsAuxE!ComSerie & " " & RsAuxE!ComNumero), TipoDocumento.CompraNotaCredito, RsAuxE!EmbArbitraje, labBcoEmisor.Caption, labLC.Caption, labFormaPago.Caption, SubRubro, iBcoEm)
...
'Letra es el código de embarque. A, B, C, .......
    ...
    If Trim(tCarpetaDespachante.Text) <> "" Then RsEmbarque!EmbCarpetaDesp = Trim(tCarpetaDespachante.Text) Else RsEmbarque!EmbCarpetaDesp = Null
                    ...
                    If Not IsNumeric(.EditText) Then Exit Sub
                        For I = 1 To .Rows - 1
                            If Val(.Cell(flexcpValue, I, 0)) = Val(.Cell(flexcpValue, Row, 0)) And .Cell(flexcpForeColor, I, 0) = vbBlack And Val(.Cell(flexcpValue, Row, 0)) > 0 Then
                                ...
                                .Cell(flexcpText, I, 3) = Format(curSuma, FormatoMonedaP)
...
Dim Resultado As String
    ...
    Cons = "Select ArtCodigo, 'Código' = ArtCodigo, Nombre = ArtNombre" _
        & " From Articulo" _
        & " Where ArtNombre LIKE '" & Trim(Articulo) & "%'"
...
On Error GoTo ErrCC
    ...
    Do While Not rsCont.EOF
        With vsContenedor
            ...
            If Not IsNull(rsCont!ECoLinea) Then .Cell(flexcpText, .Rows - 1, 2) = Trim(rsCont!ECoLinea)
        End With
        rsCont.MoveNext
    Loop
    rsCont.Close
...
Dim bSinPrecio As Boolean
    ...
    For lPos = 1 To vsContenedor.Rows - 1
        If Val(vsContenedor.Cell(flexcpText, lPos, 0)) > 0 Then
            Cons = "Select * From FleteEmbarque " _
    ...
    Next
    If bSinPrecio Then
        MsgBox "Para algún contenedor no se encontro precio.", vbExclamation, "ATENCIÓN"
    End If
    If cSuma <> Val(tFlete.Text) Then
