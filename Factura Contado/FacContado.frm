...
Begin VB.Form FacContado 
   ...
   Begin VB.Menu MnuOpciones 
      ...
      Begin VB.Menu MnuEmitir 
         ...
         Enabled         =   0   'False
      End
      Begin VB.Menu MnuOptEmitirConDescuento 
         Caption         =   "Emitir &factura con descuento"
         Enabled         =   0   'False
         Visible         =   0   'False
      End
...
'            " And AEsEstado = 1 And AEsDocumento Is Null"
    ...
    Else
        ...
        If objLista.RetornoDatoSeleccionado(6) Then
            'ES COMBO.
            miRenglon.IDCombo = miRenglon.IDArticulo
...
'            rsC.Close
        ...
        If miRenglon.IDCombo > 0 Then
            ...
            InsertoArticulosDelCombo iVarP, idEspecifico
        Else
        ...
        MnuOptEmitirConDescuento.Enabled = True
...
Private Sub Form_Activate()
    Debug.Assert TasadeCambio(paMonedaDolar, paMonedaPesos, Date - 1)
...
'    MnuOptTransacciones.Visible = miConexion.AccesoAlMenu("Cajero redpagos")
    ...
    If Me.prmIDCliente > 0 Then txtCliente.CargarControl (prmIDCliente)
...
On Error GoTo errL
    'FirmarDocumento 15897558
    Exit Sub
errL:
    MsgBox Err.Description, vbCritical, "FIRMAR"
End Sub
...
Private Sub MnuLimpiar_Click()
    ...
    MnuOptEmitirConDescuento.Enabled = False
...
Private Sub MnuOptEmitirConDescuento_Click()
    AccionEmitir  'True
End Sub
...
Private Sub AccionEmitir() '(ByVal conDescuento As Boolean)
    ...
    If ControloDatos(bNoVaRUT) Then
        ...
        If txtCliente.Cliente.Tipo = TC_Persona And oCliente.ID > 0 And CCur(labSubTotal.Caption) > Format(prmImporteConInfoCliente, "#,##0.00") And (txtCliente.Cliente.Documento = "" Or Replace(Replace(txtCliente.Cliente.Documento, ".", ""), "-", "") = "00000000" Or Replace(Replace(txtCliente.Cliente.Documento, ".", ""), "-", "") = "99999999") And lblRucPersona.Caption = "" Then
...
'            'Tengo que pedir la factura a la cual se le piensa hacer nota.
'            Dim oDocNota As New clsDocumentoCGSA
'            Dim oCQP As New clsCobrarConQuePaga
'            Set oCQP.Conexion = cBase
'            Set oDocNota = oCQP.TomarDescuentoPorNota
'            If (oDocNota Is Nothing) Then
'                MsgBox "No asocio un documento para realizarle nota.", vbExclamation, "ATENCIÓN"
'                Exit Sub
'            Else
'                'CONDICION NECESARIA ES QUE EL DESCUENTO SEA MENOR O IGUAL AL TOTAL DE LA NUEVA FACTURA.
'                If (oDocNota.Total > CCur(labTotal.Caption)) Then
'                    MsgBox "El importe de la nota debe ser menor o igual al importe del nuevo contado.", vbExclamation, "ATENCIÓN"
'                    Exit Sub
'                End If
'            End If
'        End If
...
Private Function ArticuloMueveStock(ByVal IDArticulo As Long) As Boolean
     'EsTipoDeServicio = (InStr(1, "," & tTiposArtsServicio & ",", "," & idTipo & ",") > 0)
     ArticuloMueveStock = False
...
Dim iQTot As Long
    ...
    With vsGrilla
        For I = 1 To .Rows - 1
            ...
            If ArticuloMueveStock(CLng(.Cell(flexcpData, I, 0))) Then
...
'AND RenArticulo IN (" & sIDsArts & ")"
    Cons = "SELECT DocCodigo" & _
        ...
        " INNER JOIN Articulos ON ArtID = RenArticulo AND ArtEsServicio = 0" & _
...
Dim Suma As Currency
    ...
    If (Suma > 40000 * paValorUIUltMes) Then
        MsgBox "La factura supera las 40000 UI, no puede emitirla.", vbExclamation, "ATENCIÓN"
        Exit Function
    End If
...
Private Sub GraboFacturaRenglon(ByVal bSucVtaXMayor As Boolean, ByVal NoVaRUT As Boolean) ', ByVal resQV As Byte, ByVal subRespQV As Byte, ByVal docDto As clsDocumentoCGSA)
...
sPaso = "1"
    ...
    With vsGrilla
        For I = 1 To .Rows - 1
            If CInt(.Cell(flexcpText, I, 0)) > 0 Then
                If (CCur(.Cell(flexcpText, I, 3)) < 0) Then
                    MsgBox "No puede facturar artículos en negativo.", vbExclamation, "ATENCIÓN"
                    Screen.MousePointer = 0: Exit Sub
                End If
            End If
        Next
    ...
    If strCodigoEnvio <> "" Then
        ...
        Cons = "Select EnvCodigo From Envio Where EnvCodigo IN (" & strCodigoEnvio & ") And EnvTipoFlete IN(" & paNoFletesVta & ") AND EnvFormaPago = " & TipoPagoEnvio.PagaDomicilio
        Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
        If Not RsAux.EOF Then
        
            MsgBox "Seleccionó cobrar en domicilio a un tipo de flete que no lo admite, edite el envío y cambie la forma de pago.", vbExclamation, "Tipo de flete incorrecto"
        
'            Dim iResult As VbMsgBoxResult
'            iResult = MsgBox("Seleccionó cobrar en domicilio a un tipo de flete que no lo admite." & vbCrLf & "¿Confirma continuar?", vbQuestion + vbYesNoCancel + vbDefaultButton3, "Tipo de flete incorrecto")
'            Do While iResult = vbCancel
'                iResult = MsgBox("Puso cobrar en domicilio el flete a un tipo de flete que no lo admite." & vbCrLf & "¿Confirma continuar?", vbQuestion + vbYesNoCancel + vbDefaultButton3, "Tipo de flete incorrecto")
'            Loop
...
'            End If
    ...
    If (cMoneda.ItemData(cMoneda.ListIndex) = 2) Then
        cTC = TasadeCambio(paMonedaDolar, paMonedaPesos, Date - 1)
    End If
...
'    Dim docNota As clsDocumentoCGSA
'    Set docNota = docDto
'    If docDto Is Not Nothing Then
'        With docNota
'
'        End With
'    End If
...
'FIN Obtención CAE.
    ...
    With doc
        ...
        If .Moneda.Codigo = paMonedaDolar Then .TC = cTC
...
'            If CLng(.Cell(flexcpData, I, 1)) <> TipoArticulo.Servicio And _
            ...
            If CLng(.Cell(flexcpData, I, 1)) <> TipoArticulo.Servicio And ArticuloMueveStock(CLng(.Cell(flexcpData, I, 0))) Then  'And Not EsTipoDeServicio(CLng(.Cell(flexcpData, I, 3))) Then
    ...
    If lUsuInh > 0 Then
        ...
        clsGeneral.RegistroSucesoAutorizado cBase, gFechaServidor, TipoSuceso.FacturaArticuloInhabilitado, paCodigoDeTerminal, lUsuInh, NroDocumento, Descripcion:=Mid(aTexto, 1, 50), Defensa:=Trim(sDefInh), Valor:=1, idCliente:=txtCliente.Cliente.Codigo, idautoriza:=idAutInh
    End If
...
'    End If
    ...
    resM = vbYes
    sPaso = EmitirCFE(doc, CAE, prmURLFirmaEFactura)
    If sPaso <> "" Then
        sPaso = EmitirCFE(doc, CAE, "http://192.168.1.11//WSFirmarEFactura/WSFirmarEFactura.svc?singleWsdl")
        If sPaso <> "" Then
            MsgBox "ATENCIÓN no se firmó el documento" & sPaso, vbExclamation + vbYesNo, "ATENCIÓN"
        End If
    End If
'    Do While sPaso <> ""
'        resM = MsgBox("ATENCIÓN no se firmó el documento" & vbCrLf & vbCrLf & "Presione SI para reintentar" & vbCrLf & " Presione NO para abandonar " & vbCrLf & vbCrLf & "Detalle " & sPaso, vbExclamation + vbYesNo, "ATENCIÓN")
...
'    Loop
    ...
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    If Not RsAux.EOF Then EjecutarApp App.Path & "\Instaires.exe", "doc:" & CStr(NroDocumento)
    RsAux.Close
    ...
    If Me.prmIDCliente > 0 Then Unload Me
    Exit Sub
    ...
    Do While strAux <> ""
        ...
        Do While Not RsAux.EOF
            ...
            If RsAux!EnvFormaPago = TipoPagoEnvio.PagaAhora Then
                ...
                Cons = "SELECT * FROM TipoFlete INNER JOIN Articulos ON TFlArticulo = ArtID " _
                    & " INNER JOIN TipoIVA ON ArtTipoIVA = IVACodigo " _
                    & " Where TFlCodigo = " & RsAux!EnvTipoFlete
                ...
                If Not bolInserte Then
                    With vsGrilla
                        ...
                        cIVA = Format(RsArt("IVAPorcentaje"), "#,##0.00")  'Format(IVAArticulo(RsArt!ArtId), "#,##0.00")
                        aValor = RsAux!EnvValorFlete
                ...
                If Not IsNull(RsAux!EnvValorPiso) Then
                    ...
                    Cons = "SELECT * FROM Articulos " _
                    & " INNER JOIN TipoIVA ON ArtTipoIVA = IVACodigo " _
                    & " Where ArtID = " & paArticuloPisoAgencia
                    Set RsArt = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
                    ...
                    With vsGrilla
                        ...
                        If Not bolInserte Then
                            ...
                            aValor = RsAux!EnvValorPiso
                            cIVA = RsArt("IVAPorcentaje") 'IVAArticulo(RsArt!ArtId)
'                            If MnuOpFactSinIVA.Checked = False Then
                                .Cell(flexcpText, .Rows - 1, 4) = Format(RsArt("IVAPorcentaje"), "#,##0.00")  'IVAArticulo(RsArt!ArtId)
 '                           Else
...
Dim aValor As Currency
    ...
    If (CostoServicio < 0) Then
    
        MsgBox "ATENCIÓN!!!, se cambia el precio del artículo: " & vsGrilla.Cell(flexcpText, vsGrilla.Rows - 1, 1) & " para aplicar el descuento en la reparación", vbExclamation, "ATENCIÓN"
        
        With vsGrilla
            'IVA
            .Cell(flexcpData, .Rows - 1, 4) = CCur(.Cell(flexcpData, .Rows - 1, 4)) + CostoServicio      'Guardo el costo unitario.
            .Cell(flexcpText, .Rows - 1, 3) = Format(CCur(.Cell(flexcpText, .Rows - 1, 3)) + CostoServicio, "#,##0.00")
            
            .Cell(flexcpText, .Rows - 1, 5) = Format(CCur(.Cell(flexcpText, .Rows - 1, 5)) + CostoServicio, "#,##0.00")
            .Cell(flexcpText, .Rows - 1, 6) = "No"
            
            labIVA.Caption = Format(CCur(labIVA.Caption) - (Abs(CCur(CostoServicio)) - (CCur(Abs(CostoServicio)) / CCur(1 + (CCur(.Cell(flexcpText, .Rows - 1, 4)) / 100)))), "#,##0.00")
...
Dim idAux As Integer
    ...
    For I = 1 To vsGrilla.Rows - 1
        If Trim(vsGrilla.Cell(flexcpText, I, 6)) = "Si" And (Val(vsGrilla.Cell(flexcpData, I, 1)) = TipoArticulo.Articulo Or Val(vsGrilla.Cell(flexcpData, I, 1)) = TipoArticulo.Especifico) _
            And ArticuloMueveStock(CLng(vsGrilla.Cell(flexcpData, I, 0))) Then  '  Not EsTipoDeServicio(Val(vsGrilla.Cell(flexcpData, I, 3))) Then
...
Dim cAuxPrecio As Currency, cSumaPrecio As Currency
    ...
    Else
        cSumaPrecio = BuscoDescuentoCliente(miRenglon.IDCombo, oCliente.Categoria, miRenglon.PrecioOriginal, Val(tCantidad.Text))
...
Private Sub InsertoArticulosDelCombo(ByVal varPEspecifico As Currency, ByVal idEspecifico As Long)
...
'    Cons = "Select * From PresupuestoArticulo, Articulo Where PArPresupuesto = " & miRenglon.IDCombo & " And PArArticulo = ArtID"
    ...
    Do While Not RsAux.EOF
        ...
        If iArtDto = 0 Then
            ...
            If (varPEspecifico > 0) Then
                iArtDto = RsAux("ACoArticulo")
            Else
                If cAuxPrecio * RsAux!ACoCantidad > Abs(varPEspecifico) Then iArtDto = RsAux("ACoArticulo")
            End If
        End If
        iQArt = iQArt + 1
        RsAux.MoveNext
    ...
    'si tengo que restar el precio lo divido entre la cantidad de artículos que tiene
...
'    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    Do While Not RsAux.EOF
        ...
        cAuxPrecio = BuscoDescuentoCliente(RsAux!ArtId, oCliente.Categoria, cAuxPrecio, Val(tCantidad.Text) * RsAux!ACoCantidad)
        cAuxPrecio = (CCur(cAuxPrecio) * RsAux!ACoCantidad)
        
        If (iArtDto = 0) Then
            If varPEspecifico > 0 Then
                cAuxPrecio = cAuxPrecio + varPEspecifico
            Else
                If (Abs(varPEspecifico) <= cAuxPrecio) Then
                    cAuxPrecio = cAuxPrecio + varPEspecifico
                    varPEspecifico = 0
                Else
                    varPEspecifico = varPEspecifico + cAuxPrecio
                    cAuxPrecio = 0
                End If
            End If
        ElseIf iArtDto = RsAux("ACoArticulo") Then
            cAuxPrecio = cAuxPrecio + varPEspecifico
            varPEspecifico = 0
        End If
        
        cComboParcial = cComboParcial + cAuxPrecio
        CargoArticuloEnGrilla RsAux!ArtId, RsAux!ArtTipo, CInt(tCantidad.Text) * RsAux!ACoCantidad, IIf(idEspecifico > 0, Especifico, Articulo), miRenglon.Precio, RsAux!ArtNombre, "", cAuxPrecio, cEnvio.Text, False, RsAux!ArtCodigo, idEspecifico
...
'        .Cell(flexcpFontStrikethru, I, 1) = False
        ...
        If ArticuloMueveStock(CLng(.Cell(flexcpData, I, 0))) Then '   Not EsTipoDeServicio(CLng(.Cell(flexcpData, I, 3))) Then
            If Val(.Cell(flexcpData, I, 8)) = 0 And InStr(1, paCategoriaDistribuidor, "," & oCliente.Categoria & ",") > 0 Then
        ...
        For iCont = 1 To .Rows - 1
             ' .Cell(flexcpData, .Rows - 1, 3)                tipo del artículo.
                If CLng(.Cell(flexcpData, iCont, 1)) <> TipoArticulo.Servicio And _
                    ArticuloMueveStock(CLng(.Cell(flexcpData, iCont, 0))) Then
...
'Private Sub FirmarDocumento(ByVal doc As Long)
'
'    With New clsCGSAEFactura
...
'        Set .Connect = cBase
'        MsgBox .FirmarUnDocumento(doc), vbInformation, "atención"
'    End With
'End Sub

Private Function EmitirCFE(ByVal Documento As clsDocumentoCGSA, ByVal CAE As clsCAEDocumento, ByVal urlFirma) As String
On Error GoTo errEC
    EmitirCFE = ""
    With New clsCGSAEFactura
        .URLAFirmar = urlFirma 'prmURLFirmaEFactura
        .TasaBasica = TasaBasica
        .TasaMinima = TasaMinima
        .ImporteConInfoDeCliente = prmImporteConInfoCliente
        Set .Connect = cBase
