...
Screen.MousePointer = 11
    ...
    Cons = "Select * From RenglonEnvio INNER JOIN Articulos ON ArtID = REvArticulo AND ArtEsServicio = 0 Where REvEnvio IN (" _
    ...
    FechaDelServidor
    'Marco el stock.
    Cons = "Select * From RenglonVtaTelefonica INNER JOIN Articulos ON ArtID = RVTArticulo AND ArtEsServicio = 0 " _
...
On Error GoTo errQuery
    ...
    '" WHERE VTeAnulado IS NULL and VTeTipo IN (7, 32, 33, 44) AND VTeDocumento IS NULL and VTeFechaLlamado < DATEADD(d, -" & txtDias.Text & ", GetDATE()) "
    Cons = "SELECT VTeCodigo, VTeFechaLlamado Fecha, Case WHEN VTeTipo = 44 THEN 'Redpagos' WHEN VTeTipo = 32 THEN 'WEB AC' WHEN VTeTipo =  33 THEN 'Web' WHEN VTeTipo = 7 THEN 'Telefónica' END Tipo " & _
            ...
            " WHERE VTeAnulado IS NULL and VTeTipo IN (44) AND VTeDocumento IS NULL and VTeFechaLlamado < DATEADD(d, -" & txtDias.Text & ", GetDATE()) " & _
            " ORDER BY VteCodigo"
    Set RsAux = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
    Do While Not RsAux.EOF
        With vsGrid
