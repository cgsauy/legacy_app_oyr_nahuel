...
Private Sub AccionGrabar()
    ...
    If sNuevo Then rsAux!EMsUsuario = paCodigoDeUsuario
    ...
    If Not sModificar Then gIdServer = 0
    AccionCancelar
...
Private Sub AccionEliminar()
    ...
    bHay = False
    cons = "Select * from EMailDireccion Where EMDServidor = " & gIdServer
    Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
    If Not rsAux.EOF Then bHay = True
    rsAux.Close
    
    If bHay Then
        MsgBox "Hay direcciones de correo que pertenecen al servidor seleccionado." & Chr(vbKeyReturn) & "No podrá eliminarlo.", vbExclamation, "Hay Direcciones Ingresadas"
    ...
    cons = "Select * from EMailServer Where EMSCodigo = " & gIdServer
    Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
...
Private Sub tNombre_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then Foco tHost
End Sub
...
Private Function ValidoCampos() As Boolean
    ...
    cons = "Select * from EMailServer " & _
               ...
               " And EMSCodigo <> " & gIdServer
    Set rsAux = cBase.OpenResultset(cons, rdOpenForwardOnly, rdConcurValues)
    If Not rsAux.EOF Then
        MsgBox "Ya existen servidores de correco con el nombre o dirección ingresada." & vbCrLf & "Verifique en la lista de datos.", vbExclamation, "Coincidencia de datos"
...
Private Sub DeshabilitoIngreso()
    ...
    tHost.Enabled = False: tHost.BackColor = Inactivo
        
End Sub

Private Sub HabilitoIngreso()

    tNombre.Enabled = True: tNombre.BackColor = Colores.Blanco
    tHost.Enabled = True: tHost.BackColor = Colores.Blanco
    
End Sub

Private Sub LimpioFicha()
    tNombre.Text = "": tHost.Text = ""
End Sub
