...
Private Sub Form_Load()
    ...
    CaeUso = caeG.ObtenerTipoUso(cBase)
    Set caeG = Nothing
            ...
            Case vbKeySubtract
                If CLng(lvArticulo.SelectedItem.Text) > 0 Then
                    If CantidadArticulosEnFicha(Mid(lvArticulo.SelectedItem.Key, 2)) > 0 Then
                        If lvArticulo.SelectedItem.Text = CLng(lvArticulo.SelectedItem.Text) <= CantidadArticulosEnFicha(Mid(lvArticulo.SelectedItem.Key, 2)) Then
...
Dim bInstalacion As Boolean ', bInsRealizada As Boolean
    ...
    Else
        If RsAux!DocFModificacion = CDate(labImporteTotal.Tag) Then
            ...
            'Notas de débitos.
            Dim tipoCAE As Byte
            tipoCAE = IIf((txtCliente.Cliente.Tipo = TC_Empresa And txtCliente.Cliente.Documento <> "") Or (txtCliente.Cliente.Tipo = TC_Persona And txtCliente.Cliente.RutPersona <> ""), 113, 103)
            Dim cae As New clsCAEDocumento
            Dim caeG As New clsCAEGenerador
            Set cae = caeG.ObtenerNumeroCAEDocumento(cBase, tipoCAE, CaeUso, paCodigoDeSucursal)
...
'            If sErrEfactura <> "" Then EnvioALog sErrEfactura & " Documento=" & lnDocumento
...
Private Function ExisteInstalacion(ByVal lArtID As Long) As Boolean
...
On Error GoTo errEC
    With New clsCGSAEFactura
        Set .Connect = cBase
        If Not .GenerarEComprobante(idDocumento, cae) Then
