...
'    Msg = vbNullString
    ...
    If bRemitoDeMenos Then
        ...
        With docDeMenos
            ...
            .Numero = CAEDeMenos.Numero
            .Serie = CAEDeMenos.Serie
            .Moneda.Codigo = 1
            .Total = 0
            .IVA = 0
            .Sucursal = paCodigoDeSucursal
            .Digitador = CInt(tUsuario.Tag)
            .Comentario = "Corrección traslado: " & Val(tCodigo.Tag)
            .Vendedor = CInt(tUsuario.Tag)
            .Adenda = "Traslado de mercadería: " & Val(tCodigo.Tag) & "<BR/>" & "Origen: " & cDestino.Text & ", Destino: " & labOrigen.Caption & "<BR/>" & "Memo: correción mercadería de menos."
        End With
        Set docDeMenos.Conexion = cBase
        docDeMenos.Codigo = docDeMenos.InsertoCabezalDelDocumento
        
        Set oTraslDeMenos = New clsTraspaso
        With oTraslDeMenos
            .Comentario = "Corrección traslado " & Val(tCodigo.Tag) & " artículos que llegaron de menos."
    ...
    If bRemitoMas Then
        ...
        docDeMas.Codigo = docDeMas.InsertoCabezalDelDocumento
    ...
    With vsConsulta
        For I = 1 To .Rows - 1
            ...
            If CLng(.Cell(flexcpText, I, 0)) <> CLng(.Cell(flexcpData, I, 2)) Then
                ...
                Dim iTipoAltaBaja As Integer
                Set oRenTras = New clsRenglonTraspaso
                Dim idTraMoF As Long
                If CLng(.Cell(flexcpText, I, 0)) > CLng(.Cell(flexcpData, I, 2)) Then
                    'Llegaron 10 y el traslado era por 8
                    oRenTras.Cantidad = CLng(.Cell(flexcpText, I, 0)) - CLng(.Cell(flexcpData, I, 2))
                    iTipoAltaBaja = -1
                    oTraslDeMas.Renglones.Add oRenTras
                    docDeMas.Renglones.Add oRenDoc
                    idTraMoF = oTraslDeMas.Codigo
                Else
                    'Llegaron 8 y eran 10.
                    oRenTras.Cantidad = CLng(.Cell(flexcpData, I, 2)) - CLng(.Cell(flexcpText, I, 0))
                    iTipoAltaBaja = 1
                    oTraslDeMenos.Renglones.Add oRenTras
                    docDeMenos.Renglones.Add oRenDoc
                    idTraMoF = oTraslDeMenos.Codigo
                End If
                
                With oRenTras
                    .Articulo = RsMerc("RTrArticulo")
                    .Estado = RsMerc("RTrEstado")
                    .Pendientes = 0     'YA LO PONGO ENTREGADO.
                End With
                
                oRenDoc.Cantidad = oRenTras.Cantidad
                oRenDoc.Articulo.ID = oRenTras.Articulo
                oRenDoc.EstadoMercaderia = oRenTras.Estado
                
                MarcoMovimientoStockFisicoEnLocal TipoLocal.Deposito, rs!TraLocalOrigen, oRenTras.Articulo, oRenTras.Cantidad, oRenTras.Estado, iTipoAltaBaja
                MarcoMovimientoStockFisico CLng(tUsuario.Tag), TipoLocal.Deposito, rs!TraLocalOrigen, oRenTras.Articulo, oRenTras.Cantidad, oRenTras.Estado, iTipoAltaBaja, TipoDocumento.Traslados, idTraMoF
                
                MarcoMovimientoStockFisicoEnLocal TipoLocal.Deposito, rs!TraLocalDestino, oRenTras.Articulo, oRenTras.Cantidad, oRenTras.Estado, (iTipoAltaBaja * -1)
                MarcoMovimientoStockFisico CLng(tUsuario.Tag), TipoLocal.Deposito, rs!TraLocalDestino, oRenTras.Articulo, oRenTras.Cantidad, oRenTras.Estado, (iTipoAltaBaja * -1), TipoDocumento.Traslados, idTraMoF
            End If
            
            'Hago el movimiento final por lo que era el traslado.
            If Not IsNull(rs!TraLocalIntermedio) Then
                'Quito mercadería del camión
                MarcoMovimientoStockFisicoEnLocal TipoLocal.Camion, rs!TraLocalIntermedio, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), -1
                MarcoMovimientoStockFisico CLng(tUsuario.Tag), TipoLocal.Camion, rs!TraLocalIntermedio, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), -1, TipoDocumento.Traslados, rs!TraCodigo
            Else
                'Este caso es local origen. al no haber camión --> no hubo mov de stock
                MarcoMovimientoStockFisicoEnLocal TipoLocal.Deposito, rs!TraLocalOrigen, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), -1
                MarcoMovimientoStockFisico CLng(tUsuario.Tag), TipoLocal.Deposito, rs!TraLocalOrigen, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), -1, TipoDocumento.Traslados, rs!TraCodigo
            End If
            'Hago los movimientos para el destino.
            MarcoMovimientoStockFisicoEnLocal TipoLocal.Deposito, paCodigoDeSucursal, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), 1
            MarcoMovimientoStockFisico CLng(tUsuario.Tag), TipoLocal.Deposito, paCodigoDeSucursal, CLng(.Cell(flexcpData, I, 0)), RsMerc!RTrCantidad, CLng(.Cell(flexcpData, I, 1)), 1, TipoDocumento.Traslados, rs!TraCodigo
        ...
        'GRABO LOS RENGLONES DEL TRASLADO.
        If bRemitoDeMenos Then oTraslDeMenos.InsertarRenglonesEnBD: docDeMenos.InsertoRenglonDocumentoEstadoBD
        If bRemitoMas Then oTraslDeMas.InsertarRenglonesEnBD: docDeMas.InsertoRenglonDocumentoEstadoBD
    ...
    Dim sPaso As String
    If bRemitoDeMenos Then
        sPaso = EmitirCFE(docDeMenos, CAEDeMenos)
        If sPaso <> "" Then
            MsgBox "ATENCIÓN no se firmó el documento: " & sPaso & vbCrLf & vbCrLf & "DEBE COMUNICARLO A UN ADMINISTRADOR", vbCritical, "ATENCIÓN"
        Else
            ImprimoEFactura docDeMenos.Codigo
        End If
    End If
    If bRemitoMas Then
        sPaso = EmitirCFE(docDeMas, CAEDeMas)
        If sPaso <> "" Then
            MsgBox "ATENCIÓN no se firmó el documento: " & sPaso & vbCrLf & vbCrLf & "DEBE COMUNICARLO A UN ADMINISTRADOR", vbCritical, "ATENCIÓN"
        Else
            ImprimoEFactura docDeMas.Codigo
        End If
    End If
...
Private Sub Imprimo()
    ...
    If paPrintConfD <> "" Then
        SeteoImpresoraPorDefecto paPrintConfD
    Else
        MsgBox "Mal configurada la impresora.", vbExclamation, "ATENCIÓN"
        Exit Sub
    End If
...
On Error GoTo ErrPBI
    ...
    With vsListado
        ...
        .Orientation = orPortrait
        .Device = paPrintConfD
        If .Device <> paPrintConfD Then MsgBox "Ud no tiene instalada la impresora para hoja blanca. Avise al administrador.", vbExclamation, "ATENCIÒN"
        If .PaperBins(paPrintConfB) Then .PaperBin = paPrintConfB Else MsgBox "Esta mal definida la bandeja de hoja blanca para grillas en su sucursal, comuniquele al administrador.", vbInformation, "ATENCIÓN": paPrintConfB = .PaperBin
    End With
    Exit Sub
ErrPBI:
...
'    RsMensaje!MenCategoria = ParametrosSist.ObtenerValorParametro(CategoriaMensajeStock).Valor  'paCategoriaMensajeStock
