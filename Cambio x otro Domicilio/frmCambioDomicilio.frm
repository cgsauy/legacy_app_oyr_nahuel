...
Begin VB.Form frmCambioDomicilio 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Cambio en domicilio"
   ClientHeight    =   6090
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   8445
   ...
   EndProperty
   Icon            =   "frmCambioDomicilio.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6090
   ...
   Begin orUserDigit.UserDigit txtAutoriza 
      Height          =   285
      Left            =   960
      TabIndex        =   9
      Top             =   4920
   ...
   Begin prjHiperLink.orHiperLink hliDocumento 
      ...
      EndProperty
      ForeColorOver   =   16711680
      MouseIcon       =   "frmCambioDomicilio.frx":030A
      MousePointer    =   99
      BeginProperty FontOver {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   8.25
   ...
   Begin VB.PictureBox picTitulo 
      ...
      Begin VB.PictureBox Picture1 
         ...
         Height          =   360
         Left            =   120
         Picture         =   "frmCambioDomicilio.frx":0624
         ScaleHeight     =   360
         ScaleWidth      =   360
         TabIndex        =   14
         Top             =   120
   ...
   End
End
Attribute VB_Name = "frmCambioDomicilio"
...
Dim sPaso As String
    ...
    Do While sPaso <> ""
        resM = MsgBox("ATENCIÓN no se firmó el documento" & vbCrLf & vbCrLf & "Presione SI para reintentar" & vbCrLf & " Presione NO para abandonar " & vbCrLf & vbCrLf & "Información: " & sPaso, vbExclamation + vbYesNo, "ATENCIÓN")
...
Private Function GrabarRemitosEnBD(ByVal cliente As clsClienteCFE) As Boolean
   ...
   On Error GoTo ErrResumo
    ...
    Dim docRemitoEnt As clsDocumentoCGSA
    Set docRemitoEnt = New clsDocumentoCGSA
    ...
    With docRemitoEnt
        ...
        '.Comentario = "Cambio envío: " & sIDsEnvios & ". " & txtMemo.Text
        .Vendedor = Val(txtUser.Tag)
    End With
    Set docRemitoEnt.Conexion = cBase
    docRemitoEnt.Codigo = docRemitoEnt.InsertoDocumentoBD(0)
    ...
    With docRemitoRet
        ...
        .Digitador = CInt(txtUser.Tag)
        '.Comentario = "Cambio envío: " & sIDsEnvios & ". " & txtMemo.Text
        .Vendedor = CInt(txtUser.Tag)
    End With
    
    For Each oRenDoc In docRemitoRet.Renglones
    ...
    'Para cada artículo a entregar tengo que reservar el stock.
    Dim oStock As New clsStockVenta
    Set oStock.ConnectStock = cBase
    oStock.EstadoArtEntrega = 274
    For Each oRenDoc In docRemitoEnt.Renglones
        oStock.Stock_MovimientosVenta (txtUser.Text), oRenDoc.Articulo.ID, 0, oRenDoc.Cantidad, 0, docRemitoEnt.Tipo, docRemitoEnt.Codigo
    Next
    ...
    Do While Not rsEnv.EOF
        ...
        cBase.Execute "INSERT INTO EnviosRemitos (EReEnvio, EReRemito, EReFactura, EReAutorizo) VALUES (" & rsEnv("EnvCodigo") & ", " & docRemitoRet.Codigo & ", " & Val(hliDocumento.Tag) & ", " & txtAutoriza.UserID & ")"
        
        rsEnv.Edit
        rsEnv("EnvFormaPago") = 2
        rsEnv("EnvDocumento") = docRemitoEnt.Codigo
    ...
    EmitirCFERepetitivo docRemitoEnt.Codigo
...
Public Sub MostrarAyuda(Optional msg As String = "")
    If msg = "" Then
        Select Case Me.ActiveControl.Name
            ...
            Case txtAutoriza.Name
                lblAyuda.Caption = "Ingrese el usuario que autorizó el cambio."
            Case txtUser.Name
...
Private Sub butAceptar_Click()
    ...
    If (txtAutoriza.UserID < 1) Then
        MsgBox "Ingrese el usuario que autoriza.", vbExclamation, "Validación"
        txtAutoriza.SetFocus
        Exit Sub
    End If
    
    If Not (butAceptar.Enabled) Then Exit Sub
...
errValidar:
    clsGeneral.OcurrioError "Error al validar para grabar: " + sPaso, Err.Description
    butAceptar.Enabled = True
    Exit Sub
...
Dim sPaso As String
    ...
    With lstArticulos
        ...
        .RowHeightMin = 285
        .FixedCols = 0
        .FormatString = ">Cambia|>Compró|<Artículo"
        .ColWidth(0) = 1000
        .ColWidth(1) = 1000
        .ColWidth(2) = 2000
        .ExtendLastCol = True
...
On Error Resume Next
    If KeyAscii = vbKeyReturn Then txtAutoriza.SetFocus
End Sub
...
Private Sub txtAutoriza_AfterDigit()
On Error Resume Next
    txtUser.SetFocus
End Sub
