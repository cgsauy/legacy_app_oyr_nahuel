...
Private Function fnc_GetUsuario(Optional bMsg As Boolean = True) As Boolean
    tDigito.Tag = ""
    Dim oUID As New clsUsuario
    If oUID.BuscarDigito(tDigito.Text) Then
        If oUID.Habilitado Then
            tDigito.Tag = oUID.Codigo
            tNombre.Text = oUID.Identificacion
            If bMsg Then
                With tNombre
            ...
            End If
        Else
            If bMsg Then MsgBox "Usuario no habilitado.", vbExclamation, "Atención"
        End If
    Else
        If bMsg Then MsgBox "No existe un usuario para el dígito ingresado.", vbExclamation, "ATENCIÓN"
    End If
    Set oUID = Nothing
    fnc_GetUsuario = (Val(tDigito.Tag) > 0)
End Function
...
Private Sub tDigito_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn And IsNumeric(tDigito.Text) Then fnc_GetUsuario
End Sub
...
Private Sub AccionGrabar()
    ...
    If tDigito.Tag = "" Then
        If Not fnc_GetUsuario(False) Then Exit Sub
    End If
...
errGrabar:
    Screen.MousePointer = 0
    objG.OcurrioError "Error al asignar las propiedades de la sección."
End Sub

Private Sub tTecla_KeyDown(KeyCode As Integer, Shift As Integer)

    Select Case KeyCode
        Case vbKeyF1: tTecla.Tag = 1: tTecla.Text = "F1"
        Case vbKeyF2: tTecla.Tag = 2: tTecla.Text = "F2"
        Case vbKeyF3: tTecla.Tag = 3: tTecla.Text = "F3"
        Case vbKeyF4: tTecla.Tag = 4: tTecla.Text = "F4"
        Case vbKeyF5: tTecla.Tag = 5: tTecla.Text = "F5"
        Case vbKeyF6: tTecla.Tag = 6: tTecla.Text = "F6"
        Case vbKeyF7: tTecla.Tag = 7: tTecla.Text = "F7"
        Case vbKeyF8: tTecla.Tag = 8: tTecla.Text = "F8"
        Case vbKeyF9: tTecla.Tag = 9: tTecla.Text = "F9"
        Case vbKeyF11: tTecla.Tag = 11: tTecla.Text = "F11"
        Case vbKeyF12: tTecla.Tag = 12: tTecla.Text = "F12"
    End Select
    With tTecla
        .SelStart = 0: .SelLength = Len(.Text)
    End With
