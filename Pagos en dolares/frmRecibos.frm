...
Private Sub bAgregar_Click()
    ...
    With vsLista
        ...
        aValor = TipoDocumento.CompraCredito: .Cell(flexcpData, .Rows - 1, 0) = aValor
        ...
        .Cell(flexcpText, .Rows - 1, 8) = Format(CCur(lFImporteD.Caption) * CCur(lFTC.Caption), FormatoMonedaP)
    ...
    'Busco las Notas de Credito para la compra ingresada.------------------------
    Dim bEsta As Boolean
    cons = "Select * from Compra, CompraPago " & _
               " Where CPaDocASaldar = " & Val(tFNumero.Tag) & _
               " And CPaDocQSalda = ComCodigo And ComTipoDocumento = " & TipoDocumento.CompraNotaCredito & _
               " And CPaAmortizacion < 0 "
    Set rsAux = cBase.OpenResultset(cons, rdOpenDynamic, rdConcurValues)
    Do While Not rsAux.EOF
        bEsta = False
        With vsLista
            For I = 1 To .Rows - 1
                If .Cell(flexcpValue, I, 4) = rsAux!ComCodigo Then
                    bEsta = True: Exit For
                End If
            Next
            If Not bEsta Then
                ...
                aValor = TipoDocumento.CompraNotaCredito: .Cell(flexcpData, .Rows - 1, 0) = aValor
                ...
                .Cell(flexcpText, .Rows - 1, 4) = Format(rsAux!ComCodigo, "#,##0")
                .Cell(flexcpText, .Rows - 1, 5) = Format(0, FormatoMonedaP)
                .Cell(flexcpText, .Rows - 1, 6) = Format(0, FormatoMonedaP)
                .Cell(flexcpText, .Rows - 1, 7) = Format(rsAux!ComTC, "#.000")
                
                .Cell(flexcpText, .Rows - 1, 8) = Format(rsAux!ComImporte * rsAux!ComTC, FormatoMonedaP)
                .Cell(flexcpBackColor, .Rows - 1, 2, , 3) = Colores.Inactivo
            End If
        End With
        rsAux.MoveNext
...
Private Sub ActualizoDivisaPaga()
    ...
    For I = 1 To .Rows - 1
        If .Cell(flexcpData, I, 0) = TipoDocumento.CompraCredito Then
...
Private Sub CargoDatosFactura(Codigo As Long)
    ...
    If Not IsNull(rsAux!ComSaldo) Then
        ...
        Dim bContinua As Boolean: bContinua = True
        
        If rsAux!ComSaldo = 0 Then
            If MsgBox("El saldo de la factura seleccionada es cero. " & vbCrLf & _
                            "Verifique los pagos de esta factura antes de continuar." & vbCrLf & vbCrLf & _
                            "Para agregarla a la lista presione 'si'", vbExclamation + vbYesNo + vbDefaultButton2, "Saldo Cero") = vbNo Then bContinua = False
        End If
...
Private Sub InicializoGrillas()
    ...
    With vsLista
        .Rows = 1: .Cols = 1
        .Editable = False
        .FormatString = "<Numero|<Fecha|>Saldo Dólares|>Saldo Pesos|>Id_Compra|>Paga|>Saldo Actual|>TC|>Cta.Corriente $ |"
        .ExtendLastCol = True
        .WordWrap = True
...
Private Function HayPago() As Currency
    ...
    With vsLista
        For I = 1 To .Rows - 1
            If IsNumeric(.Cell(flexcpText, I, 2)) Then
                'No concidero las notas
                If .Cell(flexcpValue, I, 5) > 0 Then aRetorno = aRetorno + .Cell(flexcpValue, I, 5)
            End If
        Next
    End With
    HayPago = aRetorno
