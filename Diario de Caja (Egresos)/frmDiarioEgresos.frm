    ...
    Do While Not rsAux.EOF
        ...
        If rsAux!TMDListado Then
            ...
            If aMovimiento <> rsAux!MDiID Then
                ...
                If Not IsNull(rsAux!CheSerie) Then
                    cons = "Select * from ChequePago Where CPaIDCheque = " & rsAux!CheID
                    If Not IsNull(rsAux!ComCodigo) Then cons = cons & " And CPaIDCompra = " & rsAux!ComCodigo
                    
                    Set rs1 = cBase.OpenResultset(cons, rdOpenForwardOnly, rdConcurValues)
                    If Not rs1.EOF Then aTexto = " (" & Format(rs1!CPaImporte, FormatoMonedaP) & ")" Else aTexto = ""
                    .Cell(flexcpText, .Rows - 1, 3) = Trim(rsAux!CheSerie) & " " & rsAux!CheNumero & aTexto
                    rs1.Close
                ...
                If Not IsNull(rsAux!MDiComentario) Then
                    ...
                    End If
                Else
                    If Trim(rsAux!ComComentario) <> Trim(aTexto) And Trim(UCase(rsAux!ComComentario)) <> "N/D" Then
                        If aTexto <> "" Then aTexto = aTexto & " // "
                        aTexto = aTexto & Trim(rsAux!ComComentario)
                    Else
                        If Trim(aTexto) = "" Then aTexto = Trim(rsAux!TMDNombre)
                    End If
                End If
                mTotalG_ME = 0
                If Not IsNull(rsAux!ComCodigo) Then
                    ...
                    End If
                Else
                    If rsAux!MDRHaber <> rsAux!MDRImportePesos Then
                        aTexto = "TC " & Format(rsAux!MDRImportePesos / rsAux!MDRHaber, "0.0##") & "  " & aTexto
                        mTotalG_ME = rsAux!MDRHaber
                    End If
                End If
...
Dim rs1 As rdoResultset
    ...
    aDisponibilidad = 0
    cons = "Select CheID, CheIdDisponibilidad, CheSerie, CheNumero, CheLibrado, CheImporte, " _
                    & " Sum(IsNull(MDRHaber, 0)) - Sum(IsNull(MDRDebe, 0)) as DH " _
           & " from  MovimientoDisponibilidad, MovimientoDisponibilidadRenglon, Cheque " _
           & " Where MDIId = MDRIDMovimiento " _
           & " And MDRHaber <> NULL" _
    ...
    Do While Not rsAux.EOF
        ...
        If rsAux!DH <> rsAux!CheImporte Then
            .Cell(flexcpText, .Rows - 1, 5) = "FALTAN"
            .Cell(flexcpText, .Rows - 1, 6) = Format(rsAux!DH, FormatoMonedaP)
            .Cell(flexcpText, .Rows - 1, 7) = "INCOMPLETO !!!"
            .Cell(flexcpBackColor, .Rows - 1, 0) = &HC0&: .Cell(flexcpBackColor, .Rows - 1, 5, , 7) = &HC0&: .Cell(flexcpForeColor, .Rows - 1, 5, , 7) = vbWhite
            .Cell(flexcpFontBold, .Rows - 1, 5, , 7) = True
        End If
...
Private Sub InicializoGrilla()
    ...
    With vsConsulta
        ...
        .ColWidth(0) = 850: .ColWidth(1) = 2700: .ColWidth(2) = 1200: .ColWidth(3) = 1450
        .ColWidth(4) = 1100: .ColWidth(6) = 950: .ColWidth(7) = 1400: .ColWidth(8) = 1100: .ColWidth(9) = 4400
