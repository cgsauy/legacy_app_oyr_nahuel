...
On Error GoTo ErrLoad
    ...
    cSucursal.Text = "Todos"
    
    'Por defecto pongo todos.
'    For I = 0 To cSucursal.ListCount - 1
'        If Val(cSucursal.ItemData(I)) = 0 Then cSucursal.ListIndex = I: Exit For
'    Next
...
Private Sub AccionConsultar2()
    ...
    Cons = "SELECT Documento.*, Renglon.*, Articulo.*, CodTipo = EnvCodigo, DpeTipo Tipo, SucAbreviacion, IsNull(CamNombre, '') as CamNombre, CASE WHEN DPeTipo = 1 THEN EnvFechaEntregado ELSE VisFModificacion END as FEnt , IsNull(EnvCamion, 0) as CamCod, CASE WHEN DPeTipo = 1 THEN EnvFechaPrometida ELSE VisFecha END as FProm, TDoSigno " _
        & "FROM DocumentoPendiente " _
        & "INNER JOIN Documento ON DPeDocumento = DocCodigo And DocAnulado = 0 LEFT OUTER JOIN Renglon ON DocCodigo = RenDocumento " _
        & "INNER JOIN TipoDocumento ON TDoID = DocTipo " _
        & "LEFT OUTER JOIN Articulo ON RenArticulo = ArtID INNER JOIN Sucursal ON SucCodigo = DocSucursal " _
...
Dim aDoc As Long, aArt As Long
    ...
    Do While Not RsAux.EOF
        ...
        If Not IsNull(RsAux("ArtID")) Then aArt = RsAux!ArtID
        ...
        If Not ExisteEnGrilla(aDoc, aArt) Then
            With vsGrilla
                ...
                If Not IsNull(RsAux("ArtCodigo")) Then
                    ...
                    .Cell(flexcpText, .Rows - 1, 6) = Format(RsAux!RenCantidad * RsAux("TDoSigno") * (RsAux!RenPrecio), FormatoMonedaP)
                ElseIf RsAux("Doctipo") = 5 Then
                    .Cell(flexcpText, .Rows - 1, 4) = "Cobranza de cuota"
                    'Es recibo.
                    .Cell(flexcpText, .Rows - 1, 6) = Format(RsAux("DocTotal"), FormatoMonedaP)
                End If
                If Not IsNull(RsAux("CamNombre")) Then .Cell(flexcpText, .Rows - 1, 7) = Trim(RsAux!CamNombre)
