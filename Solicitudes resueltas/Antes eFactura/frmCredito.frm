...
Private prmArticulosDeServicio As String
...
Private Sub bValidar_Click()
    ...
    'Controles eFactura.
    If (oCliente.Tipo = TipoCliente.Empresa) Or (oCliente.Tipo = TipoCliente.Cliente And (CCur(lTotal.Caption) > prmImporteConInfoCliente Or oCliente.RUC <> "")) Then
        ...
        If (oCliente.RUC = "" And oCliente.Cedula = "") Then
...
'                End If
    ...
    If aSolicitudEstado = EstadoSolicitud.Condicional Then
        FechaDelServidor
        AccionValidar sIDsRefinanciacion
    Else
        If aSolicitudEstado = EstadoSolicitud.Aprovada Then
            ...
            If MsgBox("La solicitud ha sido aprobada. " & "¿Desea facturarla? " & mTPrinters, vbQuestion + vbYesNo, "Facturar Solicitud") = vbYes Then
                ...
                If HayEnviosAProcesar Then
                    ProcesoEnvios
                    If MsgBox("¿Confirma realizar la emisión de las facturas?" & mTPrinters, vbQuestion + vbYesNo, "Emitir Facturas") = vbYes Then AccionFacturar sIDsRefinanciacion
                Else
                    AccionFacturar sIDsRefinanciacion
                End If
...
On Error GoTo errCargar
    ...
    If cDireccion.ListIndex <> -1 Then
        ...
        If Not oCliente Is Nothing Then
            oCliente.DireccionFactura = lDireccion.Caption
        End If
        Screen.MousePointer = 0
    End If
...
Private Function CargoObjetoContacto() As clsContacto
   
    Dim oContacto As New clsContacto
    With oContacto
        .ID = RsAux("CliCodigo")
        .Tipo = RsAux("CliTipo")
        ...
        Select Case RsAux!CliTipo
            Case TipoCliente.Cliente
                If Not IsNull(RsAux!CliCIRuc) Then .Cedula = RsAux("CliCiRUC")
                .Apellido1 = Trim(RsAux("CPeApellido1"))
                If Not IsNull(RsAux("CPeApellido2")) Then .Apellido2 = Trim(RsAux!CPeApellido2)
                .Nombre1 = Trim(RsAux("CPeNombre1"))
                If Not IsNull(RsAux("CPeNombre2")) Then .Nombre2 = Trim(RsAux!CPeNombre2)
                If Not IsNull(RsAux!CPERuc) Then .RUC = RsAux("CPeRUC")
            ...
            Case TipoCliente.Empresa
                ...
                If Not IsNull(RsAux!CEmNombre) Then
                    .Nombre1 = Trim(RsAux!CEmNombre)
                Else
                    .Apellido1 = Trim(RsAux!CEmFantasia)
                End If
        End Select
    End With
...
Private Sub CargoDatosClienteTitular(ByVal idCliente As Long)
    ...
    Cons = "Select * from Cliente, CPersona, CEmpresa" _
    ...
    If oCliente.Tipo = TipoCliente.Empresa Then
        bEmpleo.Enabled = False
        oDireccion.Value = vbChecked
    Else
        oDireccion.Value = vbUnchecked
        If oCliente.Cedula <> "" Then lCi.Caption = clsGeneral.RetornoFormatoCedula(oCliente.Cedula)
    End If
    
    lNombre.Caption = oCliente.MostrarComo
    If oCliente.RUC <> "" Then lRuc.Caption = clsGeneral.RetornoFormatoRuc(oCliente.RUC)
    ...
    End If
    If Not IsNull(RsAux!CliCategoria) Then gCategoriaCliente = RsAux!CliCategoria Else gCategoriaCliente = 0
    RsAux.Close
    
    CargoDireccionesAuxiliares oCliente.ID

    cEMailsT.CargarDatos idCliente
...
Private Sub AccionValidar(ByVal sIDsRefinanciacion As String)
...
Dim sFacturar As Boolean
    ...
    Else
        If sFacturar Then
            ...
            If HayEnviosAProcesar Then
                ProcesoEnvios
                If MsgBox("Confirma realizar la emisión de las facturas. ", vbQuestion + vbYesNo, "ATENCIÓN") = vbYes Then AccionFacturar sIDsRefinanciacion
            Else
                AccionFacturar sIDsRefinanciacion
            End If
        End If
...
Private Sub AccionFacturar(ByVal idsDocRefinanciados As String)
...
Dim aSTR As String
    ...
    If chkVencidaTit.Value = 1 Then cBase.Execute "EXEC prg_PagaVencidas " & gCliente & ", 1"
    If Val(chkVencidaConyuge.Tag) > 0 And chkVencidaConyuge.Value = 1 Then cBase.Execute "EXEC prg_PagaVencidas " & Val(chkVencidaConyuge.Tag) & ", 1"
...
'    Dim sErrEfactura As String, sLstDocs As String
'    sLstDocs = aListaDeImpresion
'    Do While Trim(sLstDocs) <> ""
'        sErrEfactura = EmitirCFE(Mid(sLstDocs, 1, InStr(sLstDocs, "E") - 1))
'        If sErrEfactura <> "" Then
'            EnvioALog sErrEfactura
'        End If
'        sLstDocs = Trim(Mid(sLstDocs, InStr(sLstDocs, ":") + 1, Len(sLstDocs)))
'    Loop
...
Dim aCantidadCuotas As Integer 'Cantidad de cuotas del plan
    ...
    oFact = GraboDatosTDocumento(Total, Iva, Plan)
    aDocumentoFactura = oFact.idFactura
    ...
    If EmitirCFE(oFact.idFactura, oFact.CAE) <> "" Then RsAux.Edit
...
Private Function GraboDatosTDocumento(MTotal As Currency, MIva As Currency, idPlan As Long) As typCredito
...
'    mFechaRetira = tFRetiro.Text
    ...
    Dim tipoCAE As Byte
    tipoCAE = IIf(((oCliente.Tipo = 2 Or oCliente.Tipo = 1) And oCliente.RUC <> ""), 111, 101)
    ...
    Set RsAux = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurValues)
    GraboDatosTDocumento.idFactura = RsAux(0)
    RsAux.Close
    '-----------------------------------------------------------------------------------------------------
...
Dim aSaldo As Currency
    ...
    Cons = " Insert Into Credito " _
        ...
        & "  CreCumplimiento, CrePuntaje, CreCliente, CreGarantia, CreTipo, CreValorCuota, CreFormaPago)" _
        & " Values (" _
        & factura & ", " _
        & Plan & ", "
    ...
    Cons = Cons & cPago.ItemData(cPago.ListIndex) & ")"         'Forma de Pago
...
Dim aFletes As String
    ...
    Cons = "Select TCuCodigo, TCuAbreviacion, PViPlan From PrecioVigente, TipoCuota" _
            ...
            & " And PViTipoCuota <> " & paTipoCuotaContado
    If gCategoriaCliente = 0 Or gCategoriaCliente = paCategoriaCliente Then Cons = Cons & " And TCuEspecial = 0"
...
Private Function CalculoPrecioConEntrega() As Boolean
    ...
    iAuxiliar = CCur(BuscoDescuentoCliente(CLng(tArticulo.Tag), gCategoriaCliente, iAuxiliar, _
            ...
            If Trim(itmX.SubItems(4)) = "" Then
                ...
                For Each itmP In lvVenta.ListItems
                    If itmX.Text = itmP.Text Then   'El mismo Tipo de Cuota
                        ...
                        'Veo si tiene descuento
                        iAuxiliar = CCur(BuscoDescuentoCliente(ArticuloDeLaClave(itmP.Key), gCategoriaCliente, iAuxiliar, _
...
Dim bHayEnvios As Boolean
    ...
    Do While Trim(Lista) <> ""
        ...
        AccionImprimirFactura iFactura, iEntrega, iEnvio, FormulasCr, bHayEnvios
...
Dim idClienteCredito As Long 'Creo esto para solucionar los casos que imprime el documento de otro cliente
    ...
    Dim oCliFact As clsContacto
    Cons = "Select * from Cliente, CPersona, CEmpresa" _
           & " Where CliCodigo = " & idClienteCredito _
           & " And CliCodigo *= CPeCliente " _
           & " And CliCodigo *= CEmCliente "
    Set RsAux = cBase.OpenResultset(Cons, rdOpenForwardOnly, rdConcurValues)
    Set oCliFact = CargoObjetoContacto()
    RsAux.Close
        ...
        Select Case LCase(NombreFormula)
            ...
            Case "cliente":
                If oDireccion.Visible And oDireccion.Value = vbChecked Then
                    ...
                    Result = crSeteoFormula(iJobCre%, NombreFormula, "'" & Trim(oCliFact.MostrarComo) & " (" & Trim(cDireccion.Text) & ")" & "'")
                Else
                    Result = crSeteoFormula(iJobCre%, NombreFormula, "'" & Trim(oCliFact.MostrarComo) & "'")
                End If
                
            Case "cedula": Result = crSeteoFormula(iJobCre%, NombreFormula, "'" & Trim(clsGeneral.RetornoFormatoCedula(oCliFact.Cedula)) & "'") 'lCi.Caption
...
'                If Trim(lRuc.Caption) <> "S/D" Then sTexto = Trim(clsGeneral.RetornoFormatoRuc(lRuc.Caption)) Else sTexto = ""
                sTexto = clsGeneral.RetornoFormatoRuc(oCliFact.RUC)
...
Private Function fnc_EsDelTipoServicio(idArticulo As Long) As Boolean
    ...
    If prmArticulosDeServicio = "" Then
        Dim miRs As rdoResultset
        
        prmArticulosDeServicio = "|"
        
        Cons = "Select ArtID from Articulo Where ArtTipo = " & paTipoArticuloServicio
        Set miRs = cBase.OpenResultset(Cons, rdOpenDynamic, rdConcurValues)
        Do While Not miRs.EOF
            prmArticulosDeServicio = prmArticulosDeServicio & CStr(miRs!ArtID) & "|"
            miRs.MoveNext
        Loop
        miRs.Close
    End If
    
    fnc_EsDelTipoServicio = (InStr(prmArticulosDeServicio, "|" & CStr(idArticulo) & "|") <> 0)
...
Dim iQInst As Currency
    ...
    If porPlan = 0 Then
        For arrIdx = LBound(arrCreditos) To UBound(arrCreditos)
            If arrCreditos(arrIdx).idFactura = porFactura Then
...
Private Function EmitirCFE(ByVal idDocumento As Long, ByVal CAE As clsCAEDocumento) As String
On Error GoTo errEC
    With New clsCGSAEFactura
        Set .Connect = cBase
        .URLAFirmar = prmURLFirmaEFactura
        Dim oPrm As New clsParametroImpresion
        If cDireccion.ListIndex > -1 Then oPrm.IDDireccion = cDireccion.ItemData(cDireccion.ListIndex)
        oPrm.idDocumento = idDocumento
        oPrm.NombreCliente = Trim(lNombre.Caption)
        If oDireccion.Visible And oDireccion.Value = vbChecked Then
            oPrm.NombreCliente = oPrm.NombreCliente & " (" & Trim(cDireccion.Text) & ")"
        End If
        If Not .GenerarEComprobante(CAE, oPrm) Then
